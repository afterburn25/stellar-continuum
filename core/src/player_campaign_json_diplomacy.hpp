...
double number(const Json &value) { return value.get<double>(); }
template <class T>
std::optional<T> opt(const Json &value, std::string_view key) {
  const auto &member = value.at(key);
  return member.is_null() ? std::nullopt : std::optional<T>(member.get<T>());
}
template <class Enum> Enum en(const Json &value, std::string_view key) {
  return static_cast<Enum>(value.at(key).get<int>());
}
