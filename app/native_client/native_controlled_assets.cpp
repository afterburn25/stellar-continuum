...
std::string role(stellar::core::FleetRole r){using stellar::core::FleetRole;switch(r){case FleetRole::Scout:return "Scout";case FleetRole::Science:return "Science";case FleetRole::Colony:return "Colony";case FleetRole::Military:return "Military";case FleetRole::Logistics:return "Logistics";}return "Fleet";}
...
View build(const stellar::core::FreshCampaignState& world,const stellar::native_colony_roster::View& colonies,
  ...
  for(const auto& c:colonies.rows){const auto it=colony_index.find(c.colony_id);if(it==colony_index.end()||!seen.insert(c.colony_id).second)continue;
    ...
    r.tooltip=c.can_open?c.name+"\n"+c.system_name+" · "+c.population+" population\nOwned · Click to focus; double-click or › to manage.":c.reason;
    if(source.stability<.35){r.severity=2;r.tooltip+="\nCritical: colony stability below 35%.";}
    else if(source.stability<.6){r.severity=1;r.tooltip+="\nWarning: colony stability below 60%.";}
    if(source.population_millions>0&&source.stored_food_population_days_millions/source.population_millions<1.){r.severity=std::max(r.severity,1);r.tooltip+="\nFood reserves below one day.";}
  ...
  for(const auto& f:fleets.own_fleets){const auto found=fleet_index.find(f.id);if(found==fleet_index.end()||!seen.insert(f.id).second)continue;const auto& source=*found->second;
    ...
    const auto location=f.current_system_id?system_name(*f.current_system_id):"En route";
    // Each canonical FleetState currently represents one commissioned vessel.
    r.detail="1 ship · "+location;r.activity=role(f.role)+" · ";
    std::string order="Idle";
    if(source.combat&&source.combat->retreat_started)order="Retreating";
    else if(source.return_to_base_requested)order="Returning to base";
    else if(source.hold_requested)order="Holding";
    else if(source.transit_phase!=FleetTransitPhase::None||source.destination_system_id)order="Moving";
    else if(source.freight_target_outpost_id)order="Supplying outpost";
    else if(source.settlement_body_id)order="Establishing settlement";
    else if(f.science_survey&&!f.science_survey->completed&&!f.science_survey->held)order="Surveying";
    else if(f.reconnaissance&&!f.reconnaissance->completed&&!f.reconnaissance->held)order="Exploring";
    else if(source.combat){switch(source.combat->order){case MilitaryOrderType::Defend:order="Defending";break;case MilitaryOrderType::Attack:order="Attacking";break;default:break;}}
    r.activity+=order;r.tooltip=f.name+"\n"+r.detail+"\n"+r.activity+"\nFleet power: "+std::to_string(static_cast<int>(std::lround(f.combat_power)));
    if(source.return_to_base_failure_reason){r.severity=1;r.tooltip+="\nReturn route needs attention.";}
    result.rows.push_back(std::move(r));
  }
  if(yard&&yard->campaign_generation==result.generation&&yard->player_civilization_id==result.observer&&yard->orbital_shipyard_complete){
    Row r;r.key={Category::Shipyards,result.observer};r.system_id=yard->home_system_id;r.name=system_name(r.system_id)+" Orbital Shipyard";
    r.detail=std::to_string(yard->pending_build_count)+" / "+std::to_string(yard->maximum_pending_builds)+" orders";r.activity="Idle";
    const auto active=std::ranges::find_if(yard->orders,[](const auto& o){return o.active;});
    if(active!=yard->orders.end()){r.progress=std::clamp(active->progress_fraction,0.,1.);r.activity="Building "+active->design_name;}
...
void Navigator::commit_preferences(Preferences next){if(persist_&&!persist_(next)){error_="Could not save navigator preferences.";return;}preferences_=next;error_.clear();pressed_.reset();rebuild();}
...
void Navigator::render(DrawList& out,int w,int h,const Art& art){
  ...
  if(preferences_.hidden){native_menu_style::button(out,l.restore,"Assets ›",normal,l.restore.contains(pointer_),true,s);return;}
  ...
  native_menu_style::panel(out,l.panel,s);label(out,l.header,"CONTROLLED ASSETS · "+std::to_string(view_.rows.size()),cyan,normal,l.panel);
  ...
  label(out,{l.search.x+8*s,l.search.y+6*s,l.search.width-40*s,24*s},search_.empty()?"Search assets…":search_,search_.empty()?muted:ink,small,l.search);
  ...
  for(const auto& e:entries_){const float height=e.row?l.row_height:l.category_height;const UiRect r{l.list.x,y,l.list.width-5*s,height};y+=height;
    ...
    if(!e.row){const int c=static_cast<int>(e.category);const bool expanded=!preferences_.collapsed[c]||!search_.empty()||(temporary_reveal_&&temporary_reveal_->category==e.category);
      out.overlay.emplace_back(FilledRectangle{clip,{7,27,41,244}});label(out,{r.x+8*s,r.y+8*s,r.width-16*s,24*s},std::string(expanded?"⌄  ":"›  ")+names[c]+"  "+std::to_string(counts_[c])+(search_.empty()?"":" ("+std::to_string(matches_[c])+")"),cyan,small,l.list);continue;}
    ...
    label(out,{x,r.y+46*s,tw,18*s},row.progress?row.activity:row.activity+" · "+(row.controlled?"Controlled":"Owned"),row.controlled?cyan:muted,small,clip);
  ...
  if(entries_.empty())label(out,{l.list.x+10*s,l.list.y+18*s,l.list.width-20*s,150*s},view_.rows.empty()?"No controlled assets.\n\nExplore the galaxy or establish a colony to begin expanding your civilization.":"No assets match your search.",muted,normal,l.list);
