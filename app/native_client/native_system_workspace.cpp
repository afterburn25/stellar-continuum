...
SystemSpatialViewport workspace_fit(const SystemSpatialSnapshot&spatial,int width,int height,const SystemTextMeasurer&measure,const std::optional<NativeSystemTravelSnapshot>&travel,const std::vector<NativeLaneLabelMetrics>&lane_metrics){
  ...
  const auto text_extent=[&](const std::string&value){const Text label{{},value,text,13};const auto measured=measure?measure(label):TextExtent{static_cast<int>(value.size()*7u),19};if(measured.width<0||measured.height<0)throw std::runtime_error("Renderer returned invalid system label bounds.");return measured;};
  ...
  const auto fit_center=[&](float scale)->std::optional<Point>{
    ...
    for(const auto&body:spatial.bodies){if(!view.is_body_visible(spatial,body))continue;const auto point=view.world_to_screen(body.offset_x,body.offset_y);const auto radius=view.body_radius(body)*(body.sol_texture_key==std::optional<std::string>{"saturn"}?2.35f:1.f)+4.f;reserve({point.x-radius,point.y-radius,2.f*radius,2.f*radius});const auto measured=label_extents.at(body.body_id);reserve({point.x+radius+1.f,point.y-static_cast<float>(measured.height)*.5f,static_cast<float>(measured.width),static_cast<float>(measured.height)});}
  ...
  float low=.01f,high=1.15f;for(int iteration=0;iteration<24;++iteration){const auto candidate=(low+high)*.5f;if(fit_center(candidate))low=candidate;else high=candidate;}const auto center=fit_center(low).value_or(Point{center_x,center_y});return {center.x,center.y,low};
}
...
SystemWorkspaceLayout SystemWorkspaceLayout::for_viewport(int width,int height) noexcept {
  ...
  const auto left=native_navigation_content_left*
                  NativeUiLayout::for_viewport(width,height).scale;
  const UiRect controls{left,130,w-left,42};
  const UiRect panel{w-panel_width-18.f,180.f,panel_width,std::max(1.f,h-198.f)};
  return {controls,{left+8,134,118,34},{left+136,134,124,34},panel,
          {panel.x+14.f,panel.y+panel.height-48.f,panel.width-28.f,34.f},
          {left,172,std::max(1.f,panel.x-8.f-left),std::max(1.f,h-172.f)},
          {panel.x+14.f,panel.y+panel.height-88.f,panel.width-28.f,34.f}};
}
...
void NativeSystemWorkspace::refresh(NativeSystemSnapshot snapshot){
  ...
  snapshot_=std::move(snapshot);spatial_=project_system(*snapshot_);
...
void NativeSystemWorkspace::render(DrawList &out,int width,int height){artwork_ready_=true;celestial_appearance_.begin_frame();if(!snapshot_||!spatial_||!viewport_)return;resize(width,height);for(std::uint32_t i=0;i<96;++i){const auto a=mix(static_cast<std::uint32_t>(snapshot_->system_id)*131u+i);const auto b=mix(a+17u);const Point p{static_cast<float>(a%static_cast<std::uint32_t>(std::max(1,width))),static_cast<float>(b%static_cast<std::uint32_t>(std::max(1,height)))};out.world.emplace_back(Circle{p,(a&3u)?0.65f:1.1f,{102,133,169,static_cast<std::uint8_t>(70u+a%80u)}});}
  ...
  if(snapshot_->stellar_object){
    ...
    if(danger>=4.f)for(int i=0;i<128;i+=2){
      ...
      if(const auto segment=clipped({viewport_->center_x+std::cos(a)*danger,viewport_->center_y+std::sin(a)*danger},{viewport_->center_x+std::cos(b)*danger,viewport_->center_y+std::sin(b)*danger},field))
    ...
    if(physics.jet_half_angle_radians>0){
      ...
      for(const double side:{0.,std::numbers::pi})for(const double edge:{-1.,1.}){
        ...
        const Point endpoint{viewport_->center_x+static_cast<float>(std::cos(angle))*reach,viewport_->center_y+static_cast<float>(std::sin(angle))*reach};
        if(const auto segment=clipped({viewport_->center_x,viewport_->center_y},endpoint,field))out.world.emplace_back(Line{segment->first,segment->second,{143,172,255,150}});
      }
    }
  }
  std::vector<std::pair<Point,float>> visible_discs;const auto star_radius=star_screen_radius(viewport_->scale);const auto star=star_color(snapshot_->primary_stellar_class);
  const bool black_hole=snapshot_->primary_stellar_class==StellarClass::BlackHole||snapshot_->archetype==StarArchetype::BlackHole;
  float primary_visual_radius=star_radius;
  if(stellar_art_&&snapshot_->stellar_object){
    const auto& definition=stellar_object_definition(snapshot_->stellar_object->type);
    primary_visual_radius*=static_cast<float>(definition.visual_scale);
    const auto seconds=presentation_seconds();
    stellar_art_(out,{viewport_->center_x,viewport_->center_y},star_radius,*snapshot_->stellar_object,seconds,field);
    if(primary_visual_radius>=46.f&&!definition.black_hole&&!definition.remnant&&definition.type!=StellarObjectType::BrownDwarf){
      const Color color{static_cast<std::uint8_t>(definition.color[0]),static_cast<std::uint8_t>(definition.color[1]),static_cast<std::uint8_t>(definition.color[2]),255};
      celestial_appearance_.append_stellar_activity(out,{viewport_->center_x,viewport_->center_y},primary_visual_radius,{color,false,mix(static_cast<std::uint32_t>(snapshot_->system_id))},seconds,field);
    }
  }else celestial_appearance_.append_stellar_disc(out,{viewport_->center_x,viewport_->center_y},star_radius,{star,black_hole,mix(static_cast<std::uint32_t>(snapshot_->system_id))},presentation_seconds(),field);
  visible_discs.emplace_back(Point{viewport_->center_x,viewport_->center_y},primary_visual_radius*1.45f);std::vector<BodyLabelCandidate> body_labels;
  for(const auto &body:spatial_->bodies){if(!viewport_->is_body_visible(*spatial_,body))continue;const auto p=viewport_->world_to_screen(body.offset_x,body.offset_y);const auto radius=viewport_->body_radius(body);const bool ringed=body.sol_texture_key&&*body.sol_texture_key=="saturn";const auto footprint=radius*(ringed?2.35f:1.f)+4.f;if(!intersects(field,p.x,p.y,footprint))continue;visible_discs.emplace_back(Point{p.x,p.y},footprint);if(ringed)celestial_appearance_.append_ring_back(out,{p.x,p.y},radius,field);std::shared_ptr<const RgbaImage> image;if(image_provider_)image=image_provider_({snapshot_->campaign_generation,body.body_id,snapshot_->survey_level==SystemSurveyLevel::fully_surveyed,body.visual_class,body.sol_texture_key,mix(static_cast<std::uint32_t>(body.body_id)),std::atan2(-body.offset_y,-body.offset_x)});if(image_provider_&&!image&&body.visual_class!=NativeSystemBodyVisualClass::unknown_planet&&body.visual_class!=NativeSystemBodyVisualClass::unknown_moon)artwork_ready_=false;if(image)out.world.emplace_back(Image{image,{p.x-radius,p.y-radius,2*radius,2*radius},std::nullopt,{255,255,255,255},field});else out.world.emplace_back(Circle{{p.x,p.y},radius,visual_color(body.visual_class)});if(ringed)celestial_appearance_.append_ring_front(out,{p.x,p.y},radius,field);if(selected_body_id_==body.body_id)out.world.emplace_back(Circle{{p.x,p.y},radius+4,{122,230,190,90}});const Text label{{},body.label,text,13};const auto measured=text_measurer_?text_measurer_(label):TextExtent{static_cast<int>(body.label.size()*7u),19};if(measured.width<0||measured.height<0)throw std::runtime_error("Renderer returned invalid system label bounds.");const auto label_width=static_cast<float>(measured.width),label_height=static_cast<float>(measured.height);const std::array<UiRect,4> placements{UiRect{p.x+footprint+5.f,p.y-label_height*.5f,label_width,label_height},UiRect{p.x-footprint-5.f-label_width,p.y-label_height*.5f,label_width,label_height},UiRect{p.x-label_width*.5f,p.y-footprint-5.f-label_height,label_width,label_height},UiRect{p.x-label_width*.5f,p.y+footprint+5.f,label_width,label_height}};body_labels.push_back(BodyLabelCandidate{body.body_id,selected_body_id_==body.body_id,body.label,placements});}
...
void NativeSystemWorkspace::focus_selected_body(int width,int height){
  ...
  dragging_=false;pending_initial_travel_fit_=false;
}
