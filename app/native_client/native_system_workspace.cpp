...
double presentation_seconds(){static const auto epoch=std::chrono::steady_clock::now();return std::chrono::duration<double>(std::chrono::steady_clock::now()-epoch).count();}
} // namespace
...
void NativeSystemWorkspace::render(DrawList &out,int width,int height){artwork_ready_=true;celestial_appearance_.begin_frame();if(!snapshot_||!spatial_||!viewport_)return;resize(width,height);for(std::uint32_t i=0;i<96;++i){const auto a=mix(static_cast<std::uint32_t>(snapshot_->system_id)*131u+i);const auto b=mix(a+17u);const Point p{static_cast<float>(a%static_cast<std::uint32_t>(std::max(1,width))),static_cast<float>(b%static_cast<std::uint32_t>(std::max(1,height)))};out.world.emplace_back(Circle{p,(a&3u)?0.65f:1.1f,{102,133,169,static_cast<std::uint8_t>(70u+a%80u)}});}
  ...
  if(!snapshot_->archetype){out.world.emplace_back(Circle{{viewport_->center_x,viewport_->center_y},star_radius*1.45f,{star.r,star.g,star.b,35}});out.world.emplace_back(Circle{{viewport_->center_x,viewport_->center_y},star_radius,star});}
  else{const bool black_hole=snapshot_->primary_stellar_class==StellarClass::BlackHole||snapshot_->archetype==StarArchetype::BlackHole;celestial_appearance_.append_stellar_disc(out,{viewport_->center_x,viewport_->center_y},star_radius,{star,black_hole,mix(static_cast<std::uint32_t>(snapshot_->system_id))},presentation_seconds(),field);}
  visible_discs.emplace_back(Point{viewport_->center_x,viewport_->center_y},star_radius*1.45f);std::vector<BodyLabelCandidate> body_labels;
