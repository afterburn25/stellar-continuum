...
SystemWorkspaceLayout SystemWorkspaceLayout::for_viewport(int width,int height) noexcept {
  ...
  const UiRect controls{left,52.f*s,w-panel_width-left-28.f*s,36.f*s};
  const UiRect panel{w-panel_width-12.f*s,260.f*s,panel_width,std::max(1.f,std::min(540.f*s,h-272.f*s))};
  return {controls,{left+6*s,56*s,78*s,28*s},{left+90*s,56*s,98*s,28*s},panel,
          {panel.x+10*s,panel.y+panel.height-44*s,panel.width-20*s,32*s},
          {left,96*s,std::max(1.f,panel.x-8*s-left),std::max(1.f,h-168*s)},
