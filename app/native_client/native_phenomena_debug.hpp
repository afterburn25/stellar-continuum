...
public:
  ...
  bool handle(const InputEvent& e,int w,int h){
    ...
    if(e.type==InputEventType::Wheel){scroll_=std::max(0,scroll_-static_cast<int>(e.wheel_y)*2);return true;}
  ...
  void render(DrawList& out,int w,int h)const{
    ...
    std::istringstream lines(data_);std::string line;int i=0,drawn=0;while(std::getline(lines,line)){if(i++<scroll_)continue;if(drawn>=18)break;native_menu_style::text(out,{l.panel.x+18*l.s,l.panel.y+(168+drawn++*23)*l.s,l.panel.width-36*l.s,23*l.s},line,font);}
...
private:
  ...
  stellar::native_ui::Dropdown dropdown_;std::string data_;int scroll_{};int selected_{};bool navigate_{};
};
}
