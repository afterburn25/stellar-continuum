...
std::string duration(const NativeResearchNode& n){return n.cost&&std::isfinite(n.cost->estimated_years_at_full_funding)?stellar::native_campaign::format_campaign_duration(n.cost->estimated_years_at_full_funding*365.25)+" at full funding":"Duration requires staffed labs";}
}
...
void NativeResearchWorkspace::render_dashboard(DrawList& out,const ResearchWorkspaceLayout& l){
  ...
  const auto title=mode_==ResearchViewMode::Recent?"RECENTLY COMPLETED":mode_==ResearchViewMode::Favorites?"FAVORITE TECHNOLOGIES":mode_==ResearchViewMode::Completed?"COMPLETED RESEARCH":mode_==ResearchViewMode::Queue?"RESEARCH QUEUE":plan_.suggestions?"RECOMMENDED & AVAILABLE":"AVAILABLE TECHNOLOGIES";
  ...
  for(const auto& card:cards){const auto clip=intersection(card.bounds,content);extent=std::max(extent,card.bounds.y+card.bounds.height+guided_scroll_-content.y);if(!clip)continue;
    ...
    if(!card.recommended&&!section&&std::ranges::any_of(cards,[](const auto& c){return c.recommended;})){clipped_text(out,{card.bounds.x,card.bounds.y-30*s,l.graph.width-24*s,25*s},content,"OTHER AVAILABLE TECHNOLOGIES",muted,l.small_font_pixels);section=true;}
    ...
    if(card.recommended&&!row){const UiRect badge{art.x+4*s,art.y+art.height-20*s,104*s,20*s};stellar::engine::ui_skin::gradient(out,badge,{35,158,211,255},{8,81,119,255},3*s,content);clipped_text(out,{badge.x+5*s,badge.y+2*s,badge.width-10*s,badge.height},content,"RECOMMENDED",bright,std::max(10,l.small_font_pixels-2));}
    ...
    if(!row){line(duration(*n),muted,l.small_font_pixels,35*s);line(concise(n->benefits,95),positive,l.small_font_pixels,card.recommended?48*s:36*s);
      if(card.recommended&&!n->recommendation_reasons.empty()){line("Recommended because:",warning,l.small_font_pixels,20*s);line(concise(n->recommendation_reasons.front(),90),muted,l.small_font_pixels,44*s);}}
    if(mode_==ResearchViewMode::Recent&&n->recent_year)line("Completed in year "+fixed(*n->recent_year,2),muted,l.small_font_pixels,20*s);
  ...
  if(cards.empty())text(out,{content.x+18*s,content.y+18*s,content.width-36*s,96*s},mode_==ResearchViewMode::Queue?"No technologies queued. Add a known program from the inspector. Queued programs wait for their requirements and funding; blocked entries are never skipped.":mode_==ResearchViewMode::Recent?"No completion records in the saved research history yet. Established starting knowledge is listed under Completed Research.":"No technologies match this view. Change the category, search or filter.",muted,l.body_font_pixels);
...
void NativeResearchWorkspace::render_controls(DrawList& out,const ResearchWorkspaceLayout& l){
  ...
  for(int i=0;i<4;++i){UiRect r{l.view_tabs.x+i*l.view_tabs.width/4,l.view_tabs.y,l.view_tabs.width/4-4*s,l.view_tabs.height};button(r,std::array{"Guided","Tech Tree","Recent","Favorites"}[i],100+i);if(static_cast<int>(mode_)==i)fill(out,{r.x,r.y+r.height-2*s,r.width,2*s},{108,223,251,255});}
  button(l.completed,"✓ Completed Research",104);button(l.queue,"Research Queue · "+std::to_string(plan_.queue.size()),105);
  if(mode_==ResearchViewMode::Tree){button({l.graph.x+8*s,l.graph.y+6*s,100*s,30*s},"Reset view",30);button({l.graph.x+116*s,l.graph.y+6*s,108*s,30*s},"Focus selected",31);button({l.graph.x+232*s,l.graph.y+6*s,100*s,30*s},"Focus active",32);}
  else {
    button({l.toolbar.x+8*s,l.toolbar.y+6*s,90*s,30*s},"Why?",1);
    button({l.toolbar.x+104*s,l.toolbar.y+6*s,68*s,30*s},list_view_?"Cards":"List",2);
    button({l.toolbar.x+178*s,l.toolbar.y+6*s,84*s,30*s},"Assistance",3);
    button(l.sort,std::array{"Relevance ▾","Time ▾","Tier ▾","Category ▾","Name ▾"}[sort_],5);
    button(l.filter,std::array{"Available ▾","All known ▾","Recommended ▾","Researching ▾","Queued ▾","Locked ▾","Completed ▾","Favorites ▾"}[filter_],4);
  }
  if(const auto* n=selected_node()){
    ...
    if (n->active) {
      ...
      button({tree.x+tree.width+6*s,tree.y,tree.width,tree.height},"Cancel research",11,n->id,n->cancel_action.enabled);
    }
    button(tree,"View in tree",8,n->id);
  }
  native_menu_style::panel(out,l.active,s);
  text(out,{l.active.x+12*s,l.active.y+8*s,l.active.width-24*s,24*s},"ACTIVE RESEARCH  ·  "+std::to_string(window_->active_program_count)+(window_->lab_capacity_only?" programs · laboratory capacity limited":" / "+std::to_string(window_->maximum_programs.value_or(0))+" slots"),bright,l.body_font_pixels);
...
std::optional<WorkspaceCommand> NativeResearchWorkspace::handle_controls(const InputEvent& e,const ResearchWorkspaceLayout& l,int width,int height){
  ...
  for(auto hit=interface_hits_.rbegin();hit!=interface_hits_.rend();++hit)if(hit->bounds.contains(e.position)){
    ...
    else if(action==3)dropdown_.open(3,{"Off","Suggestions only"},plan_.suggestions?1:0);
    else if(action==4)dropdown_.open(1,{"Available","All known","Recommended","Researching","Queued","Locked","Completed","Favorites"},filter_);
    else if(action==5)dropdown_.open(2,{"Relevance","Research time","Tier","Category","Name"},sort_);
