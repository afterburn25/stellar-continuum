...
std::vector<const NativeShipDesign*> NativeShipyardWorkspace::filtered_designs()const{
  ...
  for(const auto& d:view_->available_designs){
    ...
    if(!query.empty()&&lower_ship(d.name+" "+role_name(d.role)+" "+d.description).find(query)==std::string::npos)continue;
    result.push_back(&d);
  }
...
UiRect NativeShipyardWorkspace::card(std::size_t i,const ShipyardWorkspaceLayout& l)const{
  ...
  return {l.designs.x+(i%columns)*(w+gap),l.designs.y+design_scroll_+(i/columns)*268*l.scale,w,260*l.scale};
}
...
std::string NativeShipyardWorkspace::batch_blocker()const{
  const auto* d=selected_design();if(!d)return "Select a ship design.";
  const auto q=std::ranges::find(d->batch_quotes,quantity_,&stellar::core::ShipbuildingBatchAssessment::quantity);
  if(q!=d->batch_quotes.end())return q->can_start?std::string{}:q->blocker.value_or("Construction unavailable.");
  // Older projection clients retain their single-order contract.
  if(quantity_==1)return d->can_start?std::string{}:d->start_blocker.value_or("Construction unavailable.");
  return "Waiting for construction quote.";
}
void NativeShipyardWorkspace::bind_preferences(std::filesystem::path path){
  ...
  }catch(const std::exception&){favorites_.clear();notice_="Ship favorites could not be loaded.";notice_accepted_=false;}
}
bool NativeShipyardWorkspace::save_preferences(){
  ...
  }catch(const std::exception&){notice_="Ship favorites could not be saved.";notice_accepted_=false;return false;}
}
...
ShipyardWorkspaceCommand NativeShipyardWorkspace::handle(const InputEvent& e,int w,int h){
  ...
  if(dropdown_.visible()){const int id=dropdown_.id();if(auto choice=dropdown_.handle(e,id==1?l.sort:l.filter,w,h)){if(id==1)sort_=*choice;else filter_=*choice;design_scroll_=0;}return {ShipyardWorkspaceCommandKind::None,true};}
  ...
  if(search_focused_&&(e.type==InputEventType::TextEntered||e.type==InputEventType::BackspacePressed)){
    ...
    design_scroll_=0;return {ShipyardWorkspaceCommandKind::None,true};
  }
  if(e.type==InputEventType::Wheel){
    if(l.designs.contains(e.position)){const auto designs=filtered_designs();const float content=designs.empty()?0:card(designs.size()-1,l).y-design_scroll_-l.designs.y+268*s;design_scroll_=std::clamp(design_scroll_+e.wheel_y*66*s,std::min(0.f,l.designs.height-content),0.f);}
    else if(l.orders.contains(e.position))order_scroll_=std::clamp(order_scroll_+e.wheel_y*50*s,std::min(0.f,l.orders.height-30*s-(view_?view_->orders.size():0)*64*s),0.f);
    else if(l.design_details.contains(e.position))detail_scroll_=std::clamp(detail_scroll_-e.wheel_y*48*s,0.f,detail_limit_);
  ...
  if(l.sort.contains(e.position)||l.filter.contains(e.position)){const bool sort=l.sort.contains(e.position);std::vector<std::string> options;if(sort)for(auto t:ship_sorts)options.emplace_back(t);else for(auto t:ship_filters)options.emplace_back(t);dropdown_.open(sort?1:2,std::move(options),sort?sort_:filter_);return {ShipyardWorkspaceCommandKind::None,true};}
  for(int i=0;i<static_cast<int>(ship_categories.size());++i){UiRect r{l.categories.x,l.categories.y+i*54*s,l.categories.width,48*s};if(r.contains(e.position)){category_=i;design_scroll_=0;return {ShipyardWorkspaceCommandKind::None,true};}}
  const auto designs=filtered_designs();for(std::size_t i=0;i<designs.size();++i){const auto r=intersection(card(i,l),l.designs);if(r&&r->contains(e.position)){selected_design_id_=designs[i]->id;selected_order_id_.reset();cancel_confirmation_id_.reset();notice_.clear();detail_scroll_=0;quantity_=1;return {ShipyardWorkspaceCommandKind::None,true};}}
  if(view_){const UiRect clip{l.orders.x,l.orders.y+30*s,l.orders.width,l.orders.height-30*s};
    for(std::size_t i=0;i<view_->orders.size();++i){const auto& o=view_->orders[i];const UiRect r{clip.x,clip.y+order_scroll_+i*64*s,clip.width,60*s};const auto hit=intersection(r,clip);if(!hit||!hit->contains(e.position))continue;
      selected_order_id_=o.order_id;selected_design_id_.reset();cancel_confirmation_id_.reset();notice_.clear();detail_scroll_=0;
...
void NativeShipyardWorkspace::render(DrawList& out,int w,int h,stellar::native_ship_ui::NativeShipArtAssets* art)const{
  ...
  text(out,l.title,"SHIP CONSTRUCTION",bright,l.title_font_pixels,FontFace::Heading);
  text(out,{l.title.x,l.title.y+31*s,l.title.width,24*s},view_?view_->yard_name+"  ·  Treasury "+view_->formatted_treasury+"  ·  Industry "+number(view_->available_industry,0):"Loading shipyard…",muted,l.small_font_pixels);
  button(l.close,"×");
  for(int i=0;i<static_cast<int>(ship_categories.size());++i)button({l.categories.x,l.categories.y+i*54*s,l.categories.width,48*s},ship_categories[i],category_==i);
  button(l.search,search_.empty()?"Search ships…":search_,search_focused_);button(l.sort,std::string(ship_sorts[sort_])+" ▾");button(l.filter,std::string(ship_filters[filter_])+" ▾");
  ...
  if(designs.empty())text(out,l.designs,search_.empty()?"No known designs match these filters.":"No known designs match your search.",muted,l.body_font_pixels);
  for(std::size_t i=0;i<designs.size();++i){const auto& d=*designs[i];const auto r=card(i,l);const auto clip=intersection(r,l.designs);if(!clip)continue;
    ...
    clipped_text({r.x+8*s,r.y+188*s,r.width-16*s,20*s},*clip,role_name(d.role),muted,l.small_font_pixels);
    ...
    clipped_text({r.x+8*s,r.y+234*s,r.width-16*s,20*s},*clip,stellar::native_campaign::format_campaign_duration(d.minimum_build_days_at_full_shipyard_rate)+" minimum",muted,l.small_font_pixels);
  }
  stellar::native_ui_style::menu_panel(out,l.orders);text(out,{l.orders.x+10*s,l.orders.y+6*s,l.orders.width-20*s,22*s},"BUILD ORDERS  /  "+std::to_string(view_?view_->orders.size():0),bright,l.small_font_pixels,FontFace::Heading);
  const UiRect queue{l.orders.x+5*s,l.orders.y+30*s,l.orders.width-10*s,l.orders.height-35*s};
  if(!view_||view_->orders.empty())text(out,queue,"No ships are under construction.",muted,l.body_font_pixels);
  if(view_)for(std::size_t i=0;i<view_->orders.size();++i){const auto& o=view_->orders[i];const UiRect r{l.orders.x,queue.y+order_scroll_+i*64*s,l.orders.width,60*s};const auto clip=intersection(r,queue);if(!clip)continue;
    ...
    clipped_text({x,r.y+27*s,r.width-(x-r.x)-122*s,18*s},queue,(o.active?"ACTIVE · ":"QUEUED · ")+number(o.progress_fraction*100,0)+"%  ·  "+stellar::native_campaign::format_campaign_duration(o.industry_remaining/stellar::core::shipbuilding_industry_per_day)+" at full production",muted,l.small_font_pixels);
  ...
  const UiRect details{l.design_details.x+10*s,l.design_details.y+8*s,l.design_details.width-20*s,l.design_details.height-16*s};float y=details.y-detail_scroll_;
  ...
  if(d){write(d->name,bright,l.body_font_pixels);if(art){if(auto image=art->image_for(d->id,d->role)){const UiRect p{details.x,y,details.width,152*s};out.overlay.emplace_back(Image{image,p,cover_source(*image,p),{255,255,255,255},details});y+=160*s;}}
    write(d->description);write("COMBAT",good);write("Hull  "+number(d->hull,0)+"     Armor  "+number(d->armor,0)+"     Shields  "+number(d->shields,0));
    write("Weapon damage  "+number(d->weapon_damage,1));write("MOBILITY",good);write("Speed  "+stellar::core::format_interstellar_metric_speed(d->strategic_speed));write("Maximum leg  "+stellar::core::format_interstellar_metric_primary(d->maximum_leg_range_light_years));write("Fuel endurance  "+stellar::core::format_interstellar_metric_primary(d->fuel_endurance_light_years));write("Sensors  "+number(d->sensor_range,1));write("Propulsion  "+d->propulsion_generation);write("Crew  "+std::to_string(d->crew)+"     Cargo  "+number(d->cargo_capacity,0));
    ...
    write(o->active?"Active construction":"Waiting for the production line",good);write("Industry remaining  "+number(o->industry_remaining,1));write("Population reserved  "+number(o->reserved_population_millions,3)+" million");write("Cancelling releases the unused authorization and reserved population under the normal construction rules.");}
  else write("Select an available design to review its requirements.",muted);
  detail_limit_=std::max(0.f,y+detail_scroll_-details.y-details.height);
  ...
  if(d&&view_){const auto q=std::ranges::find(d->batch_quotes,quantity_,&stellar::core::ShipbuildingBatchAssessment::quantity);
    ...
    readiness="Authorization "+credit+"\nIndustry "+number(industry,0)+" · "+std::to_string(quantity_)+" vessel"+(quantity_==1?"":"s")+"\n";
    const auto reason=batch_blocker();readiness+=reason.empty()?"Ready to build.":reason;
  }else if(o)readiness="Refund if cancelled now  "+o->formatted_refund+"\n"+o->cancellation_blocker.value_or("Unused authorization will be returned.");
  text(out,l.readiness,std::move(readiness),muted,l.small_font_pixels);
  if(d){button(l.minus,"−");button(l.quantity,std::to_string(quantity_));button(l.plus,"+");button(l.favorite,has_ship(favorites_,d->id)?"★ Favorite":"☆ Favorite",has_ship(favorites_,d->id));}
  ...
  if(o){enabled=o->can_cancel;action=cancel_confirmation_id_==o->order_id?"CONFIRM CANCEL + REFUND":"CANCEL ORDER";}
  else if(d){enabled=batch_blocker().empty();action=enabled?(d->will_queue?"QUEUE BUILD":"START BUILD"):"BUILD UNAVAILABLE";}
