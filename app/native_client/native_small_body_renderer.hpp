...
public:
  ...
  void clear(){cache_.clear();hits_.clear();last_scene_.reset();generation_=0;system_=-1;}
  ...
  // The last solids scene submitted to a draw list. It lets inspection tools
  // verify the emitted mesh/material instances without guessing which
  // Scene3DView in a frame belongs to small bodies (sky domes and planet
  // globes submit their own views into the same world list).
  [[nodiscard]] const std::shared_ptr<const stellar::native_map::Scene3D>& last_scene()const noexcept{return last_scene_;}
