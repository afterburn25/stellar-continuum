 ...
 private:
  ...
  void render_right(DrawList& out,const NativeColonyView& v,const PlanetaryLayout& l)const{
    ...
    if(tab_==2){
      ...
      const std::array<const char*,3> capacity_keys={"PLANET_CAP_FOOD","PLANET_CAP_WATER","PLANET_CAP_HOUSING"};const std::array<const char*,3> capacity_names={"Food","Water","Housing"};
      int capacity_index=0;
      for(const auto& pair:std::array<std::pair<const char*,double>,3>{{{"Food",v.food_capacity_millions},{"Water",v.water_capacity_millions},{"Housing",v.housing_capacity_millions}}})write(trf("PLANET_CAP_LINE",{tr(capacity_keys[capacity_index++],capacity_names[capacity_index-1]),population(pair.second),population(v.population_millions),pair.second<v.population_millions?tr("PLANET_DEFICIT","Deficit "):tr("PLANET_SURPLUS","Surplus "),population(std::abs(pair.second-v.population_millions))},"{0}  {1} / {2}\n{3}{4}"),pair.second<v.population_millions?bad:ink);
