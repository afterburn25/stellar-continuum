 ...
 bool handle(const InputEvent& e,int w,int h,stellar::core::CampaignFrame& frame){
  ...
  if(e.type==InputEventType::KeyPressed&&e.key){
   ...
   if(count){
    ...
    // Arrow scroll-follow: the ring covers rendered rows only, so Up on the
    // first row scrolls one row up and Down on the last scrolls one row
    // down — the slot stays focused and re-resolves to the newly revealed
    // row. Other nav keys leave the list to the next control.
