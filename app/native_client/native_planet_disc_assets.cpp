...
std::shared_ptr<const RgbaImage> NativePlanetDiscAssets::request_image(const SystemBodyAppearance &appearance){
  ...
  auto copied=appearance;auto root=storage_->asset_root;const auto reservation=appearance.texture_key?source_planet_disc_bytes:procedural_planet_disc_bytes;auto ticket=storage_->preparation->submit(reservation,[copied=std::move(copied),root=std::move(root),lighting_step]{return prepare_disc(copied,root,lighting_step);});if(!ticket)return {};storage_->pending.push_back({std::move(key),std::move(*ticket)});return {};
}
