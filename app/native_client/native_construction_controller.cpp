...
NativeConstructionCommandOutcome stale(const stellar::engine::LocalizationTable *l){const char *k="CONSTRUCTION_MSG_STALE";return {false,(l&&l->contains(k))?std::string(l->translate(k)):std::string("Construction changed; refresh before issuing an order."),0.};}
}
...
NativeConstructionCommandOutcome NativeConstructionController::start(CampaignFrame&f,std::uint64_t g,std::uint64_t r,std::string_view id){require_owner();if(!generation_||*generation_!=g||r!=revision_||!projected_view_||!player_species_id_)return stale(locale_);auto cur=project(f,g);const auto old=std::ranges::find(projected_view_->projects,id,&NativeConstructionProject::id),now=std::ranges::find(cur.view.projects,id,&NativeConstructionProject::id);if(old==projected_view_->projects.end()||now==cur.view.projects.end()||projected_view_->player_civilization_id!=cur.view.player_civilization_id||projected_view_->home_system_id!=cur.view.home_system_id||*player_species_id_!=cur.species||!same_currency(projected_view_->currency,cur.view.currency)||!same_action_terms(*old,*now,ConstructionOrderIntent::Start))return stale(locale_);if(!now->start.enabled)return {false,now->start.message,0.};auto c=context(f);auto result=start_construction_project(c.command,c.player.id,id);if(result.accepted){signature_.reset();projected_view_.reset();}return {result.accepted,result.message,0.};}
NativeConstructionCommandOutcome NativeConstructionController::queue(CampaignFrame&f,std::uint64_t g,std::uint64_t r,std::string_view id){require_owner();if(!generation_||*generation_!=g||r!=revision_||!projected_view_||!player_species_id_)return stale(locale_);auto cur=project(f,g);const auto old=std::ranges::find(projected_view_->projects,id,&NativeConstructionProject::id),now=std::ranges::find(cur.view.projects,id,&NativeConstructionProject::id);if(old==projected_view_->projects.end()||now==cur.view.projects.end()||projected_view_->player_civilization_id!=cur.view.player_civilization_id||projected_view_->home_system_id!=cur.view.home_system_id||*player_species_id_!=cur.species||!same_currency(projected_view_->currency,cur.view.currency)||!same_action_terms(*old,*now,ConstructionOrderIntent::Queue))return stale(locale_);if(!now->queue.enabled)return {false,now->queue.message,0.};auto c=context(f);auto result=queue_construction_project(c.command,c.player.id,id);if(result.accepted){signature_.reset();projected_view_.reset();}return {result.accepted,result.message,0.};}
NativeConstructionCommandOutcome NativeConstructionController::cancel(CampaignFrame&f,std::uint64_t g,std::uint64_t r,std::string_view id){require_owner();if(!generation_||*generation_!=g||r!=revision_||!signature_)return stale(locale_);auto cur=project(f,g);if(cur.signature!=*signature_)return stale(locale_);auto c=context(f);auto result=cancel_construction_project(c.command,c.player.id,id);if(result.accepted){signature_.reset();projected_view_.reset();}return {result.accepted,result.message,result.refunded_credits};}
}
