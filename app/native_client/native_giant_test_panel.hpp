...
class NativeGiantTestPanel {
 ...
 bool visible_{},ring_shadow_{true},planet_shadow_{true};int pressed_{-1};
 ...
 public:
 bool visible()const{return visible_;}void close(){visible_=false;pressed_=-1;}
 void open(stellar::core::CampaignFrame& frame){visible_=true;try{const auto& p=frame.runtime().world().campaign().developer_provenance;if(p&&p->giant_test)request_=p->giant_test->controls;apply(frame,request_);}catch(const std::exception& e){notice_=e.what();}}
 bool handle(const InputEvent& e,int w,int h,stellar::core::CampaignFrame& frame){
  ...
  if(e.type==InputEventType::LeftPressed){pressed_=-1;for(int i=0;i<20;++i)if(button(l,i).contains(e.position))pressed_=i;}
  ...
  try{switch(action){
   ...
   case 8:yaw_-=.3f;return true;case 9:yaw_+=.3f;return true;
   case 10:pitch_=std::clamp(pitch_-.2f,-1.5f,1.5f);return true;case 11:pitch_=std::clamp(pitch_+.2f,-1.5f,1.5f);return true;
   case 12:phase_+=std::numbers::pi/4;return true;
   ...
   case 16:ring_shadow_=!ring_shadow_;return true;case 17:planet_shadow_=!planet_shadow_;return true;
   case 18:next={};yaw_=0;pitch_=.15f;phase_=0;ring_shadow_=planet_shadow_=true;break;
   case 19:close();return true;
  }apply(frame,next);}catch(const std::exception& error){notice_=error.what();}return true;
 }
