...
void NativeStartupWorkspace::show_setup()noexcept{screen_=StartupScreen::Setup;setup_.begin_sandbox();selected_slot_.reset();load_scroll_={};failure_.clear();reset_pointer();}
void NativeStartupWorkspace::reset_pointer() noexcept{pointer_={};hover_feedback_.reset();focus_=-1;}
void NativeStartupWorkspace::show_entry() noexcept{screen_=StartupScreen::Entry;selected_slot_.reset();load_scroll_={};failure_.clear();reset_pointer();}
void NativeStartupWorkspace::set_slots(stellar::native_startup::NativeStartupSaveSlots slots){slots_=std::move(slots);selected_slot_=slots_.slots.empty()?std::nullopt:std::optional<std::size_t>{0};load_scroll_={};screen_=StartupScreen::LoadSlots;reset_pointer();}
...
std::optional<stellar::engine::AnnouncementValue> NativeStartupWorkspace::focused_value(int width,int height,const TextMeasurer&measure)const{
  return screen_==StartupScreen::Setup?setup_.focused_value(width,height,measure):std::nullopt;
}
...
StartupIntent NativeStartupWorkspace::handle(const InputEvent&e,int width,int height,const TextMeasurer&measure){
  ...
  if(screen_==StartupScreen::Entry)hover_target=hit(e.position,{l.new_campaign,l.load_campaign,l.settings,l.development,l.exit,(return_to_campaign_available_||!continue_save_.empty())?l.return_to_campaign:UiRect{}});
    ...
    if(l.list.contains(e.position))for(std::size_t i=0;i<slots_.slots.size();++i){
      const float pitch=46.f*l.scale;
      const auto row=intersect({l.list.x,l.list.y+i*pitch-load_scroll_.scroll_offset,l.list.width,pitch-4*l.scale},l.list);
  ...
  if(screen_==StartupScreen::LoadSlots&&e.type==InputEventType::Wheel&&l.list.contains(e.position)){
    const float pitch=46.f*l.scale;
    load_scroll_.sync_rows(slots_.slots.size(),pitch,l.list.height);
    load_scroll_.scroll_to(load_scroll_.scroll_offset-e.wheel_y*pitch*3.f);
