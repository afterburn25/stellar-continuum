...
| Standalone engine shell | PARTIALLY IMPLEMENTED | `app/engine_main.cpp` (`stellar-engine.exe`) links only `stellar_engine` + `stellar_native_platform` — windowed Vulkan host, JobSystem demo work, Profiler aggregates/frame graph, LocalizationTable load, diagnostics install; no game module | Manual launch; covered indirectly by `stellar_native_platform` + engine unit tests | Dashboard shell, not an editor/game template; no scene demo content, asset mounting or scripting surface yet |
