...
| Standalone engine shell | PARTIALLY IMPLEMENTED | `app/engine_main.cpp` (`stellar-engine.exe`) links only `stellar_engine` + `stellar_native_platform` — windowed Vulkan tools host (Dashboard/Assets/Profiler/Localization sidebar), JobSystem demo work, Profiler aggregates/frame graph, `VirtualizedList` asset browser with image preview, `LocalizationTable` catalog inspector, diagnostics install; no game module | Manual launch; covered indirectly by `stellar_native_platform` + engine unit tests | Tools host foundation, not a full editor: no scene viewport, content authoring, package/mod tooling or scripting surface yet |
| Native engine editor | PARTIALLY IMPLEMENTED | `app/editor_main.cpp` + `app/editor_project.cpp` (`stellar-editor.exe`) links engine + core + platform — galaxy workspace generating through authoritative `generate_stellar_catalog` on a JobSystem worker (250/500/1000/2500 sizes, seed regen), pannable/zoomable class-colored star map, click-select system inspector (class, archetype, traits, physical properties), searchable virtualized systems list, display-name/note/bookmark annotation layer, atomic JSON project save/load (`write_file_atomically`, schemaVersion 1, all-or-nothing parse); succeeds the stranded 0.1.9 WPF editor on `work/stellar-engine-editor` (PR #326) | `editor_project` document tests (round-trip + malformed rejection); generation path covered by `galaxy_catalog` family tests; manual launch for UI | Authoring foundation: no undo/redo, multi-file projects, asset embedding, or body-level editing yet |
