...
| Localization | PARTIALLY IMPLEMENTED | Engine `LocalizationTable`/`LocalizationService` (fallback chain, format/plural, reload); the startup screens, new-game setup (species details, galaxy selection, sandbox configuration), diplomacy, research, and fleet workspaces, pause menu, settings hub, and General/Audio/Video/Voice panels plus voice-cue subtitles consume `data/locale/en.json` | `localization`, `native_general_settings`, `native_audio_settings`, `native_video_settings`, `native_voice_settings`, `native_settings_hub`, `native_startup_workspace`, `native_new_game_workspace`, `native_diplomacy_workspace`, `native_research_workspace`, `native_fleet_workspace`, `native_voice_playback` | Startup chrome, setup wizard, diplomacy/research/fleet surfaces, menus, and voice cue keys resolve through the catalog, English only; controller-produced data strings and other HUD surfaces remain literal |
