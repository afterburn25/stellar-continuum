...
| Localization | PARTIALLY IMPLEMENTED | Engine `LocalizationTable`/`LocalizationService` (fallback chain, format/plural, reload); the settings hub and General, Audio, Video, and Voice panels plus voice-cue subtitles consume `data/locale/en.json` | `localization`, `native_general_settings`, `native_audio_settings`, `native_video_settings`, `native_voice_settings`, `native_settings_hub`, `native_voice_playback` | The settings surfaces and voice cue keys resolve through the catalog, English only; gameplay HUD strings remain literal |
...
- **Localization (req 25):** `LocalizationTable`/`LocalizationService` ship in
  the engine; the settings hub and all four settings panels (General, Audio,
  Video, Voice) consume the cooked `Data/locale/en.json` catalog, and minted
  voice-cue localization keys resolve through the live table at subtitle
  presentation.
