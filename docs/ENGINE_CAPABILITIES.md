...
| Asset cooker/compression | IMPLEMENTED BUT NEEDS POLISH | Engine asset cooker/texture cook/XPRESS+LZMS codecs, `StellarCooker` | `engine_asset_cooker`, image/GPU tests | HDR/mesh cooking absent; ~1,011 legitimate quality-gate fallbacks remain (LZMS cut stored bytes ~12%) |
...
| Localization | PARTIALLY IMPLEMENTED | Engine `LocalizationTable`/`LocalizationService` (fallback chain, format/plural, reload); General Settings panel consumes `Data/locale/en.json` | `localization`, `native_general_settings` | Only the General Settings surface and English ship; other UI strings remain literal |
...
| 22 | Audio engine | PARTIAL — CPU mixer | **UNCHANGED** — existing 48kHz mixer + SAPI voice retained; bus abstraction deferred. Audit note: the richer event-driven voice pipeline (`NativeGameplayVoiceBridge` → `NativeVoiceRouter` → `NativeSpeechRequest` → playback/SAPI) ships tested but has no client consumer — the live path is `NativeAudioDirector`'s `VoiceCue`/`show_caption`. `NativeSpeechRequest::localization_key` is minted (`voice.<cue>.<variant>`) but unread; adopting it means migrating the client's voice path, not a caption tweak. | `native_audio*`, `native_voice*` | `native_audio` |
...
| 24 | Advanced UI | PARTIAL — theme helpers | **PARTIAL** — `VirtualizedList`, `TableModel` (sort/filter), `TreeModel` (expand/flatten) in engine; `VirtualizedList` is the colony roster's scroll model (stride row_height, `scroll_to`/`max_scroll` clamps, wheel input). Table/Tree screen adoption pending. | `engine/…/ui_viewmodels.hpp`, `app/native_client/native_colony_roster.cpp` | `batcher_ui`, `native_colony_roster` |
| 25 | Localization | MISSING | **ADOPTED (first real consumer)** — `LocalizationTable`/`LocalizationService` (JSON locales, fallback chain, positional+named formatting, plurals, runtime reload). The General Settings panel resolves its static labels through `Data/locale/en.json`, shipped via the cooker as a packaged asset and read through `resource_stream` (identical strings preserve parity; missing keys fall back to literals). Earlier claims of a developer-tools embedded catalog were wrong — no such consumer existed. Remaining panels/gameplay strings are still literal; only English ships. | `engine/…/localization.hpp`, `data/locale/en.json`, `app/native_client/native_general_settings.*` | `localization`, `native_general_settings` |
...
- **Localization (req 25):** `LocalizationTable`/`LocalizationService` ship in
  the engine; the General Settings panel is the first real consumer via the
  cooked `Data/locale/en.json` catalog.
