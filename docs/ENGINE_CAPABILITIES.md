...
| 25 | Localization | MISSING | **ADOPTED** — `LocalizationTable`/`LocalizationService` (JSON locales, fallback chain, positional+named formatting, plurals, runtime reload). The developer tools panel resolves its chrome strings through an embedded English catalog; `load_locale_document`/`load_locale_file` let mods or locale packs override them. Parity-locked gameplay strings remain literal by design. | `engine/…/localization.hpp`, `app/native_client/native_developer_tools.cpp` | `localization`, `native_developer_tools` |
