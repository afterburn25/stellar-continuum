...
| Native engine editor | PARTIALLY IMPLEMENTED | `app/editor_main.cpp` (`stellar-editor.exe`) links engine + core + platform — galaxy workspace generating through authoritative `generate_stellar_catalog` on a JobSystem worker (250/500/1000/2500 sizes, seed regen), pannable/zoomable class-colored star map, click-select system inspector (class, archetype, traits, physical properties); succeeds the stranded 0.1.9 WPF editor on `work/stellar-engine-editor` (PR #326) | Manual launch; generation path itself covered by `galaxy_catalog` family tests | Foundation workspace: no annotation/bookmark authoring, project save/load, asset embedding, or body-level editing yet |
