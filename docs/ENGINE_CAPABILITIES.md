...
| 24 | Advanced UI | PARTIAL — theme helpers | **PARTIAL** — `VirtualizedList`, `TableModel` (sort/filter), `TreeModel` (expand/flatten) in engine; `VirtualizedList` adopted by the developer tools Diagnostics/Saves tabs (wheel scroll over overflowed rows). Table/Tree screen adoption pending. | `engine/…/ui_viewmodels.hpp`, `app/native_client/native_developer_tools.cpp` | `batcher_ui`, `native_developer_tools` |
