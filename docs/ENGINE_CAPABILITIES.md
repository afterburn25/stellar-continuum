...
- **Public interface:** `SurfaceResponse3D` gains `cloud_albedo` [0,1] —
  ...
  additional directionals and point lights.
...
- **Persistence:** entity `surface` block `{normal, properties, cloud,
  ...
  plus flat `terminatorWrap` round-trip through `Scene3dDocument`;
  `surface` requires at least one map and rejects out-of-range scalars.
  `MaterialSurface` component codecs the same fields; `spawn_scene3d`
  attaches it when any field differs from defaults and
  `scene3d_from_world` exports it back.
- **Editor:** Scene3D tool entity rows `surfaceMaps`, `surfaceShape`,
  `cloudDeck`, `terminator` edit the live preview.
- **Tests:** `native_scene3d_gpu` — deck compositing brightness,
  `cloud_albedo` scaling, wrap-diffuse lighting at the geometric
  terminator, wrap invariance at the fully lit pole; `engine_scene3d` —
  cloud-only acceptance + scalar bound rejects; `engine_project` —
  document round-trip + `surface`/`terminatorWrap` rejections;
  `engine_world` — `MaterialSurface` spawn/codec/export round-trips.
- **Limitations:** the deck is a texture-space composite — no volumetric
  cloud shells, cloud self-shadowing or gas-giant banding; wrap is a
  scalar per material, not a physically derived model.
