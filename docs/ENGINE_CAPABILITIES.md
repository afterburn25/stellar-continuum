...
| Localization | PARTIALLY IMPLEMENTED | Engine `LocalizationTable`/`LocalizationService` (fallback chain, format/plural, reload); General, Audio, Video, and Voice settings panels plus voice-cue subtitles consume `data/locale/en.json` | `localization`, `native_general_settings`, `native_audio_settings`, `native_video_settings`, `native_voice_settings`, `native_voice_playback` | All four settings surfaces and voice cue keys resolve through the catalog, English only; gameplay HUD strings remain literal |
...
- **Localization (req 25):** `LocalizationTable`/`LocalizationService` ship in
  the engine; all four settings panels (General, Audio, Video, Voice) consume
