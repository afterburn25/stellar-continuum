...
| Audio | PARTIALLY IMPLEMENTED | Engine native audio + Windows media decode; App director/settings; device-fault recovery — device-level failures (unplugged/default-device/SDL stream faults) arm a bounded 5 s retry that rebuilds `AudioOutput`, restores persisted volumes, resubmits the decode job when needed, and resumes music via the normal `menu_ready` path (`stats().device_recoveries` counts rebuilds); decode/asset failures stay permanent | `native_audio`, `native_audio_director` (injected-fault recovery + decode-no-retry), settings tests | Current-track PCM and bounded queues; incremental streaming/spatial/bus architecture unfinished |
