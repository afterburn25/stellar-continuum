...
> Historical design handoff recovered from `work/planetary-command-window`.
> The native 0.1.10 port restores the management screen, artwork and persistent
> building slots. It preserves existing native settlement starting-hub levels
> and colonization costs/timing; the new-arrival foundation rule below is not
> enabled by this UI restoration. Zero-level foundations are supported when
> present in native state. See `docs/engine/NATIVE_UI_RESTORATION_ALPHA_0110.md`
> for the shipped scope and save-format limitations.
