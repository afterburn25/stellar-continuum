...
| 2 | Simulation Scheduler + Simulation LOD | IMPLEMENTED BUT NEEDS POLISH (engine layer) — `simulation_scheduler.cpp` tier cadence + `simulation_executor.cpp` tasks/dependencies/dirty+event wakeups/budgets/JobSystem waves/per-domain stats (`simulation_executor` + `simulation_scale_250…5000` tests/benchmarks); Core phases not yet driven through it | Drive Core phases through the executor (dependency/cadence/LOD policy); deterministic pause/backlog and large-campaign parity |
...
| 30 | Engine Editor / Stellar Tools | PARTIALLY IMPLEMENTED — cooker/import CLI and Developer inspectors; `stellar-engine.exe` standalone tools host (engine-only link) and `stellar-editor.exe` native C++23 editor host ship beside the client — full authoritative world-assembly pipeline, galaxy + system-orbit workspaces, body inspection, searchable systems list, system/body name/note/bookmark annotations with document-level undo/redo, editable project name, atomic project save/load | Grow the editor toward the stranded WPF feature set (PR #326): Save-As path picker / recent files, asset embedding, property mutation tooling; unified tool document lifecycle |
