...
| 30 | Engine Editor / Stellar Tools | PARTIALLY IMPLEMENTED — cooker/import CLI and Developer inspectors; `stellar-engine.exe` standalone tools host (engine-only link) and `stellar-editor.exe` native C++23 editor host ship beside the client — galaxy workspace via authoritative generation, searchable systems list, name/note/bookmark annotations with undo/redo, atomic project save/load | Grow the editor toward the stranded WPF feature set (PR #326): multi-file projects, asset embedding, body-level editing; unified tool document lifecycle |
