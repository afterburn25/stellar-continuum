...
| Developer / demo mode | DeveloperCampaignSessionService, DeveloperTools, PlayableDemoScenario | `core/developer_*`, `native_developer_tools`, `native_development_menu`, session policy | OK | `developer_commands_parity`, `developer_campaign_json_parity`, `developer_campaign_session_parity`, `native_developer_session`, `native_development_menu` | PARITY VERIFIED | Separate slot (`developer-autosave.json` beside the player save) with envelope codec, load-or-create + `.bak` recovery + legacy `demo-autosave.json` import (never displaces a Developer save), provenance + ToolsUsed markers, six commands (`grant_resources`/`finish_orders`/`reveal_galaxy`/`unlock_technology`/`unlock_research`/`advance_30_days`), Developer frame policy + 24x Demo speed + 720d autosave, two-level DEVELOPMENT submenu (OPEN/LOAD DEVELOPER, WORLD SEED default `20260908`, arm→confirm NEW DEVELOPER CAMPAIGN preserving the old save as `.bak`, DEVELOPER TOOLS, BACK) + DEV TOOLS panel + PLAYER CAMPAIGN row, mode-aware save/load routing through `PlayerCampaignSaveController` |
...
| UI (native) | Main.*, panels | `app/native_client/*_workspace` (18+ modules) | OK | workspace + input tests + smoke validators | PARTIAL | Fleet/shipyard/research/construction/colony/surface/settlement/system/startup/diplomacy/battle workspaces + recent-events notification feed + selected-system inspection card (survey status/progress, guidance, intel facts, observer-safe colony intel; reference `Main.Inspection`/`SystemInspectionPanel`) + toggleable SUPPLY NETWORK panel (home-system logistics metrics, guidance, node rows; reference `LogisticsNetworkPanel`/`Main.Logistics`) + civilian fleet recovery controls (hold/resume/return-to-base with paid-commitment confirmation; reference `UiToggleSelectedCivilianFleetHold`/`UiRequestSelectedCivilianReturnToBase`) + empire overview in the fleet panel detail area (knowledge-gated selected system, home distance, colony quick-open into the orbital view, combined power; reference `EmpireOverviewPanel`) + toggleable MISSIONS & SETTLEMENT board (top-rail MISSIONS button; owned scout/science/colony cards with phase, destination, ETA and the ported `ExplorationMissionStatus` summary text; reference `ExplorationMissionPanel` missions tab) + its Colony Sites tab (bounded site browser over the settlement-mission controller, ship/site navigation, detail text, select-ship-on-map, owned-colony View actions) + reference-structured SYSTEM INSPECTOR body detail (survey badge, PHYSICAL/ENVIRONMENT/SATELLITES & SIGNALS sections, `MetricFormat` SI strings, Orbits/Known-moons rows, FOCUS PLANET viewport-centering action; reference `PlanetInspectorPanel`) + surface building management (UPGRADE/REPAIR/SHUT DOWN-RESTART/PRIORITIZE action grid with lock/affordability gating, condition/efficiency and powered/staffed/operating rows, hub status + UPGRADE HUB in the empty-selection inspector; reference `PlanetSurfaceView` action buttons + `UiUpgradeSurfaceBuilding`/`UiRepairSurfaceBuilding`/`UiSetSurfaceBuildingEnabled`/`UiSetSurfaceBuildingPriority`/`UiUpgradeSurfaceHub`) + owned-colony card actions (Land into the surface view for solid-surface worlds; Collect freight on resource outposts with reference `CanRequestFreight`/`FreightActionReason` gating and `FindAvailableFreighter` lowest-id idle bulk-freighter scan; `UiOpenOwnedColony(colonyId, land:true)`/`UiRequestOutpostFreight`) + toggleable ECONOMY panel (top-rail ECONOMY button; SOVEREIGN TREASURY cards: reserves/net-day/income-day/costs-day, materials storage + materials/day, treasury-health state via `assess_treasury`; INDUSTRIAL PRIORITY Balanced/Infrastructure-first/Shipbuilding-first toggles dispatched through `set_industry_priority` with reference `UiSetIndustryPriority` denial-status + accepted-change Economy notification; daily cash-flow income rows (colony economy, surface trade) and operating-cost rows (colony administration, population services, habitat support, fleet operations, orbital maintenance, surface maintenance, research programs with reserved-milestone-credits suffix) from `economy_credit_flow`; reference `Main.Economy`/`EconomyPanel`) + armed-fleet order row (HOLD/DEFEND/RETREAT strategic `MilitaryOrder`s through `issue_military_order` with core rejection surfacing, LOCATE map-centering for every owned selection, ENGAGE preserved; reference `UiIssueMilitaryOrder`/`UiFocusOwnedFleet`); the sealed fleet validator authors a `patrol_corvette_mk1` formation and requires the smoke's HOLD/DEFEND/RETREAT chain to land a durable DEFEND order in the reloaded save) done |
| Rendering (native) | Main.VisualMap, renderers | `engine/native_map_platform`, `app/native_client` scene | OK | Vulkan smoke + capture validators | PARTIAL | Galaxy art, star markers, ship art, route effects, strategic territory overlay (fills, contours, labels, fog, claim arcs, unexplored dimming), orbital construction markers + software-rasterized staged structures, surface colony scene (hub, per-type building sprites, construction phases, roads, ghost previews, cached deterministic terrain-relief hillshade sampled from the canonical `surface_terrain_height` heightfield reproducing the reference `colony_terrain.gdshader` palette model — per-class low/high/exposed tones (temperate/reducing/rocky/oceanic/frozen/hot/airless derived from the surveyed environment like `Main.Surface.cs` `SurfaceVisualClass`), body-seeded noise bands, slope-exposed rock, hub civic-paving ring with tile joints, drainage/basin/stratum modulation and the reference sun direction — clipped to the terrain rect, evidenced by `relief_images` in the surface validator), generated planet-disc sprites for fully surveyed non-Sol bodies (deterministic per-body pick from the approved `assets/visual/planets` pool by visual class; canonical Sol uses the nine approved Sol body discs in the same pre-lit format, giants/unknown stay procedural; procedural ring layers classified by `body_ring`; `NATIVE_PLANET_ART_SOURCES.md` provenance + hash-pinned packaging) + generated star-disc sprites for fully surveyed systems (per-`StellarClass` approved `assets/visual/stars` set, BlackHole/unsurveyed stay procedural; `NATIVE_STAR_ART_SOURCES.md` provenance + hash-pinned packaging) done |
...
| Video settings | MainMenuLayer VIDEO panel, VideoSettingsService | `native_video_settings` + `Window` runtime display setters | OK | `native_video_settings` CTest + `--audio-smoke` `video_settings` field | PARTIAL | Pause-menu VIDEO button: DISPLAY (borderless/exclusive fullscreen at the desktop mode), V-SYNC (Off/On/Adaptive via `SDL_SetRenderVSync`), FRAME CAP (Automatic/60/120/144/Unlimited via present throttle), reference Apply → 15s CONFIRM DISPLAY → Keep/Revert rollback, persisted `video-settings.json`. The reference's disabled RESOLUTION row and its MSAA/3D-resolution rows are 3D-pipeline only and intentionally absent |
...
- 172/172 graphical CTest (incl. `native_diplomacy_*`, `native_territory_projection`,
  `native_orbital_structure`, `native_surface_scene`, `native_surface_relief`,
  `native_audio`, `native_audio_settings`, `native_battle_workspace`,
  `native_notifications`, `native_support`, `native_voice`,
  `developer_commands_parity`, `developer_campaign_json_parity`,
  `developer_campaign_session_parity`, `native_developer_session`,
...
- Developer/demo mode (`core/developer_*` + `native_developer_tools`):
  ports `DeveloperCampaignSessionService`/`DeveloperTools`/
  `PlayableDemoScenario` end to end. The Developer slot lives beside the
  player save (`developer-autosave.json`) behind an envelope codec
  (`write_prepared_developer_campaign` over
  `PreparedPlayerCampaignSave::capture_developer`) that persists provenance
  and the ToolsUsed flag; load-or-create resolves primary → `.bak` → legacy
  `demo-autosave.json` import (only when no Developer save exists) → fresh
  demo-seed world, and explicit loads never generate a replacement.
  `PlayerCampaignSaveController` takes a developer-mode flag so autosave and
  manual saves share the same mode-aware capture; the player writer rejects
  developer captures and vice versa. Sessions derive the Developer frame
  policy (≤0.25-day substeps, ≤4/frame), the 720d/48d autosave scheduler and
  the 24x Demo speed from campaign provenance; Demo is gated out of the
  player speed cycle and keys. The pause menu gains a DEVELOPMENT row in
  both modes (plus RESUME PLAYER in Developer mode) opening the reference
  two-level submenu (`native_development_menu`): OPEN/LOAD DEVELOPER, a
  WORLD SEED field defaulting to `PlayableDemoScenario.Seed` (20260908),
  arm→confirm NEW DEVELOPER CAMPAIGN that spawns a fresh seeded Developer
  world while the previous Developer save survives as the slot's `.bak`,
  DEVELOPER TOOLS and BACK. Switching checkpoints the outgoing campaign
  then loads-or-creates the target slot. The DEV TOOLS panel lists the six
  reference commands and dispatches through `run_developer_command`, which
  runs the command through the canonical simulation (`advance_30_days`
  steps in ≤0.25-day substeps) and writes exactly one checkpoint
  afterward. `native_developer_session` covers the session boundary:
  envelope roundtrip, player-loader rejection, command acceptance, reload
  and player-mode rejection; `native_development_menu` covers submenu
  geometry, press/release pairing, seed parsing and the arm→confirm flow.
  `--developer-smoke` drives the full path in the packaged client —
  submenu entry, mode switch, Demo speed, tools command, envelope save,
