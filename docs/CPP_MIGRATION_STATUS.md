...
| Audio | AudioDirector | `engine/native_audio`, `app/native_client/native_audio_director` | OK | audio/director CTest + relocated audio startup/reload | PARTIAL | Existing score and navigation confirmation; settings, voice and full event wiring remain |
...
4. Native audio settings, voice and full event wiring remain; bounded music/SFX playback and startup/campaign navigation confirmation now work.
...
`a56351f8` passed native CI `34960835811`, covering this repair and developed-fleet checkpoint. The subsequent audio candidate needs its own CI.
