...
| Developer / demo mode | DeveloperCampaignSessionService, DeveloperTools, PlayableDemoScenario | `core/developer_*`, `native_developer_tools`, session policy | OK | `developer_commands_parity`, `developer_campaign_json_parity`, `developer_campaign_session_parity`, `native_developer_session` | PARITY VERIFIED | Separate slot (`developer-autosave.json` beside the player save) with envelope codec, load-or-create + `.bak` recovery + legacy `demo-autosave.json` import (never displaces a Developer save), provenance + ToolsUsed markers, six commands (`grant_resources`/`finish_orders`/`reveal_galaxy`/`unlock_technology`/`unlock_research`/`advance_30_days`), Developer frame policy + 24x Demo speed + 720d autosave, START/RESUME DEVELOPER ↔ RESUME PLAYER menu rows + DEV TOOLS panel, mode-aware save/load routing through `PlayerCampaignSaveController` |
...
- 171/171 graphical CTest (incl. `native_diplomacy_*`, `native_territory_projection`,
...
- Developer/demo mode (`core/developer_*` + `native_developer_tools`):
  ...
  player speed cycle and keys. The pause menu gains START/RESUME DEVELOPER
  (player mode) or DEV TOOLS + RESUME PLAYER (Developer mode) rows and a
  mode/tools-used status line; switching checkpoints the outgoing campaign
