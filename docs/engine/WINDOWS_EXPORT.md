...
Engine 0.1.50 adds three maintained local-travel launches: route preparation, canonical local motion, and paused reload. Their diagnostics must match the saved fleet identity, order revision, destination, position and clock. Known/unknown exit input and exact connected-lane membership are checked. Three new flags bring the total to 32; three local-travel captures are sidecars. Combined checks passed 130 CTest, 110 Python checks and sixteen actual Vulkan launches. Preview/export linking retains debug symbols without incremental linker caches. See NATIVE_SYSTEM_TRAVEL.md and NATIVE_BUILD_BUDGET.md.
