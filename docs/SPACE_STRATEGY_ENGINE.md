...
| 15 | GNN/public-information hooks | PARTIAL | `EventHistory::feed(observer, since, min_significance)` is the public-information substrate. **News presentation landed:** `seed_chronicle_notifications` populates the player notification feed from the persisted chronicle at campaign admission, and `native_chronicle` provides the scrollable chronicle browser (newest-first snapshot, 4000-entry cap with true total, on-demand refresh) opened from the notification panel. Remaining: category/significance filtering in the browser, voice presentation. |
