...
| 3 | Population framework | IMPLEMENTED (engine layer) | `Population` cohort model: demographics, mortality/fertility attribution, aging, employment, education migration, explicit emigrant slices; 17M-headcount scale test. `population_habitability.hpp` resolves `environment_needs` via `evaluate_habitability` (hard tag gate + unmet reasons). Colony consumption pending. See [POPULATION_FRAMEWORK.md](POPULATION_FRAMEWORK.md). |
