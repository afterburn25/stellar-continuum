...
| 15 | GNN/public-information hooks | PARTIAL | `EventHistory::feed(observer, since, min_significance)` is the public-information substrate. **News presentation landed:** `seed_chronicle_notifications` populates the player notification feed from the persisted chronicle at campaign admission (recorded dates/summaries, observer-filtered) — the 32-item transient window now shows real history after load. Remaining: scrollable chronicle browser and voice presentation. |
