...
activation. `IRangeValueProvider` went writable after that: `SetValue`
queues the value (latest wins), the owner drains it once per frame and
...
Pad pinning is now reachable in the UI: `kGamepadDeviceCount` names the
shared slot bound, `describe_binding` renders a pin as `Pad N Btn/Axis M`,
and a focused controls row cycles its gamepad-kind bindings through
any→pad 1..4→any via D or right-click — `SETTINGS_CONTROLS_DEVICE*` keys
announce the result and a hint line appears once any row binds pad input;
`native_startup_workspace` covers the cycle, wrap, key-only no-op and
pointer path. Pin notices name the connected pad through the new
`Window::gamepad_names()` platform accessor (slot-number fallback for a
free/unnamed slot) and flag a sibling action answering the same trigger
on the target pad — pinning stays deliberate, nothing is stripped. Pad UI navigation landed: `native_pad_input.hpp` translates
dpad/south/east/start presses into arrow/Return/Escape events while a
focus-ring surface owns input (`ui_owns_pad_input` mirrors the
...
translation table. Dpad auto-repeat landed: `PadNavigationRepeater`
(same header) tracks held dpad directions per pad slot and re-emits the
stored press after a 0.45 s delay at 0.09 s intervals — the emit
re-checks the ownership gate, a frame hitch reschedules instead of
bursting, a window focus drop disarms, and south/east/start stay
discrete; the startup entry loop runs the same repeater on a
steady-clock delta. Stick nav landed: `PadStickNavigator` turns a
left-stick deflection past 0.5 into a synthetic dpad press (0.25 release
hysteresis, per-device state, reversal re-engages) that flows through
...
press+release through normal dispatch (see row 26). Toggle and
writable-range patterns landed too (see row 26). Open: a full fragment
tree and non-Windows backends.
...
VirtualizedList boundary snap (row 26): the empire-monitor audit found a real dead spot — when `max_scroll` lands a fraction of a row under a whole-row multiple (`floor(91.12/22.78)` → 3.999998), `sync_rows`/`visible_range` floored `first` one row short, so the final row could never occupy a rendered slot: Down edge-scroll pinned focus on the last visible row forever, and wheel scrolling left the last row permanently unrendered for pointer users too. `VirtualizedList` now snaps offsets within 1e-3 of a row boundary onto the boundary in both `visible_range` and `sync_rows`. Pinned by `batcher_ui` (boundary max-scroll exposes the final row, plus `ScrollView::scroll_interval_into_view` unit coverage) and a 16-civilization `native_developer_index` fixture that Down-scrolls through every empire row, wraps at the true bottom, and Up-scrolls back to the first row.
HUD/session-notice localization (row 57): the remaining literal HUD strings now resolve through the catalog — the system-environment loading banner (`MAP_ENVIRONMENT_LOADING`), the central-object galaxy label (`GALAXY_CENTRAL_OBJECT`), the map-zoom readout (`HUD_MAP_ZOOM` via `trf`), the star-chart update status (`HUD_UPDATING_CHART`), the three diagnostic-export notices (`SUPPORT_*`), and the three nav labels that bypassed `tr()` (`NAV_RESEARCH`/`NAV_CONSTRUCTION`/`NAV_SHIPYARD` — keys already shipped). `SessionNotice` gained a `message_key` member: every fixed save/load/recover/exit/wait notice emits a `SESSION_NOTICE_*` key and the client translates `notice.message` through it at display time (diagnostic logs keep the literal English). Dynamic messages — failure detail text, core load-pipeline statuses — stay literal by design (the emitters live in core without a locale). The keyboard-candidate status lines (`STATUS_*` keys) also resolve at their call sites, along with the planetary order outcome notices, construction loading notice, unknown-system display name, diplomacy unsurveyed-destination notice, freight-restored notice, and the system-view phenomena band (reuses `PHENOMENA_*`); the dead `spectral_name` helper was removed. Only `outcome.message` pass-throughs (core-emitted command results), core load-progress detail, and developer-tool surfaces (English-only by convention) remain literal. 38 new keys in en.json + de.json; the `localization` parity test validates coverage and placeholders, and `native_campaign_session` pins `message_key` on the Saved and failure paths. Follow-on surfaces now resolve the same way: the campaign-feedback toast translates its category labels through `set_localization` (`FEEDBACK_*` keys, English `notice_label` fallback, bounded 3-notice window preserved); the home-logistics producer (`build_home_logistics` + `HomeLogisticsController::set_localization`) resolves node-kind labels, supply statuses, the HOME SYSTEM fallback title, the legend, and every unavailable/failure message (`SUPPLY_KIND_*`/`SUPPLY_STATUS_*`/`SUPPLY_ERR_*`/`SUPPLY_HOME_SYSTEM`/`SUPPLY_MESSAGE_LEGEND`); and `NativeSystemTravelController::set_localization` translates its four observer/survey denial strings (`TRAVEL_DENY_*`) before they reach `system_workspace_.set_notice`. All three keep raw-English fallback when no locale is bound; `native_logistics` and `native_campaign_feedback` pin catalog resolution in-memory. The controller sweep then localized every remaining fixed denial/result message and display label: `NativeFleetController` (route preview/order/recovery/military denials, `FLEET_MSG_*`), `NativeOutpostFreightController` (stale-view/economy/freighter/preflight denials, `FREIGHT_MSG_*`), `NativeSurfaceConstructionController` (eight management action labels, eleven confirmation descriptions incl. the `{n}`-day hub-expansion format, and the result messages, `SURFACE_*`), `NativeShipyardController` (yard name, unavailable-design placeholder, stale-queue denials, `SHIPYARD_*`), `NativeSystemViewController` (survey/availability denials, `SYSTEM_DENY_*`), `NativeSettlementMissionController` (ten denials plus the three live-status labels, `SETTLE_*`), and `NativeDiplomacyController` (contact names/statuses, access vocabulary, agreement status, proposal directions/summaries, `DIPLOMACY_*`). Diplomacy's filter tokens were decoupled first — `NativeDiplomacyContact::political_state` and `NativeDiplomacyAgreementRow::agreement_status` carry the raw enums so neutral/hostile/at-war filters and the active-agreement summary never compare localized text; proposal summaries handed to `send_proposal` stay literal since they persist into simulation records. `main.cpp` binds all six controllers; `native_settlement_targeting` pins `SETTLE_STATUS_READY` translation and English fallback, and the `localization` parity test covers the ~115 new keys. A follow-on pass closed the stragglers: `NativeConstructionController`'s stale-state denial (`CONSTRUCTION_MSG_STALE`), `NativeColonyController`'s four `build()` denials (`COLONY_MSG_*`), the audio/voice settings panels' status lines (`SETTINGS_AUDIO_STATUS_*`, `SETTINGS_VOICE_STATUS_*` — saved/defaults/load-failed/playback-unavailable), and `NativeVideoController::panel_notice`'s four classified notices (`SETTINGS_VIDEO_NOTICE_*`; the controller now keeps its own `locale_` alongside the forwarded view binding). The remaining literals are owner-thread/invariant exceptions, core-persisted proposal summaries, voice-dialogue cue data (resolved through `dialogue_key` cue scripts, not the locale catalog), and developer-only surfaces, plus the missions producers: `evaluate()`, `colony_site_selection()`, `build_mission_board()`, and `build_owned_colony_rows()` now take a borrowed `LocalizationTable*` (main.cpp passes the client table) and resolve every composed status/site/freight sentence through ~68 `MISSION_*`/`MISSIONS_*` keys — `native_missions` pins catalog translation of the empty-sites message and an unfunded-fleet summary while unbound calls keep the English fallbacks. A follow-on pass keyed the remaining feed messages: `NativePlayerNotification` carries `message_key`/`message_arg`, `publish_campaign_notifications` emits `NOTIFY_MSG_*` keys (count suffix via `{0}`), the diplomatic harvest emits `NOTIFY_DIP_*` keys per event kind (the secrecy-substitute signal uses `NOTIFY_DIP_SIGNAL`), and `observer_safe_fleet_message` takes a borrowed locale for its `SYSTEM_NAME_UNKNOWN` redaction token; `native_notification` pins display-time translation and `native_notification_events` pins the key/arg fields. The legacy `VoiceCue` captions followed the same shape — `VoiceCaption` gained `speaker_key`/`text_key` (`VOICE_CUE_RECON`/`RESEARCH`/`SURVEY`, `VOICE_SPEAKER_SCIENTIST`) resolved in `render_voice_caption`, which now takes the client locale; keyed cue scripts and dynamic announcements are unaffected. The remaining literals are owner-thread/invariant exceptions, core-persisted proposal summaries, voice-dialogue cue data (resolved through `dialogue_key` cue scripts, not the locale catalog), and developer-only surfaces. The `localization` test now also audits source references: a single-pass scanner walks `app/native_client/**` (comments, strings, char literals, raw strings, and digit separators handled) and requires every `ALL_CAPS_WITH_UNDERSCORES` literal to exist in `en.json` — so it covers resolver calls, `*_key` field/parameter values, `*_keys` tables, `*_key` mapper returns, and positional key arguments alike. Dynamic `PREFIX_ + value` concatenation families are skipped; five intentional non-key identifiers (the `GALAXY_PAD` input-mapper context, the `STELLAR_CONTINUUM_DEVTOOLS` env var, `MAJOR_FLARE`/`SMALL_PROMINENCE` dataset tokens, and the `SETTINGS_ACTION_` prefix) sit on an explicit allowlist. The audit immediately caught 44 keys that silently fell back to English in every shipped locale (a11y focus labels across the chronicle, roster, assets, logistics, construction, economy, inspection, research, and notifications panels; `SETTINGS_SCALE_*`/`SETTINGS_COLORBLIND_*` option names; `BATTLE_*` transport labels; `NAV_*` and `SETUP_*_LABEL` chrome) — all now cataloged in `en.json` + `de.json` with matching placeholders. A second check walks the embedded input-context JSON and requires a `SETTINGS_ACTION_<UPPER(name)>` catalog entry for every declared `InputMapper` action — it caught the three un-cataloged `GALAXY_PAD` axis actions (`map_pan_x`/`map_pan_y`/`map_zoom`), now keyed; a third pins `native_chronicle::category_label`'s return labels to `NOTIFY_CATEGORY_<UPPER(label)>` entries so a new event-domain prefix can't silently render English.
