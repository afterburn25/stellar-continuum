...
struct Window::Storage {
  ...
  int width{},height{};bool initialized{},left_down{},focused{true},minimized{},vsync{},vsync_unavailable{},exclusive_fullscreen{},text_input_requested{},text_input_active{};Point pointer{};Uint64 fallback_interval_ns{},frame_cap_interval_ns{},last_present_ns{};
  ~Storage(){
...
void Window::draw(const DrawList &draw_list,const std::optional<std::filesystem::path>&screenshot){
  ...
  const auto pace=std::max(storage_->frame_cap_interval_ns,storage_->vsync_unavailable?storage_->fallback_interval_ns:Uint64{0});if(pace){const auto now=SDL_GetTicksNS();if(storage_->last_present_ns&&now-storage_->last_present_ns<pace)SDL_DelayPrecise(pace-(now-storage_->last_present_ns));storage_->last_present_ns=SDL_GetTicksNS();}require(SDL_RenderPresent(storage_->renderer),"SDL present failed");
}
bool Window::set_vsync(const int mode){
  if(SDL_SetRenderVSync(storage_->renderer,mode)){storage_->vsync=mode!=0;storage_->vsync_unavailable=false;return true;}
  const std::string reason=SDL_GetError();storage_->vsync=false;storage_->vsync_unavailable=true;
  if(!storage_->fallback_interval_ns){float refresh=60.f;const auto display=SDL_GetDisplayForWindow(storage_->window);if(display){if(const auto *desktop=SDL_GetDesktopDisplayMode(display);desktop&&desktop->refresh_rate>1.f)refresh=desktop->refresh_rate;}storage_->fallback_interval_ns=static_cast<Uint64>(1000000000./static_cast<double>(refresh));}
  SDL_LogWarn(SDL_LOG_CATEGORY_RENDER,"Renderer VSync update failed (%s); pacing presents at the display refresh",reason.c_str());
  return false;
}
bool Window::set_fullscreen(const bool exclusive){
  if(exclusive){
    const auto display=SDL_GetDisplayForWindow(storage_->window);
    const auto *mode=display?SDL_GetDesktopDisplayMode(display):nullptr;
    if(!mode||!SDL_SetWindowFullscreenMode(storage_->window,mode))return false;
  }else if(!SDL_SetWindowFullscreenMode(storage_->window,nullptr))return false;
  if(!SDL_SetWindowFullscreen(storage_->window,true))return false;
  storage_->exclusive_fullscreen=exclusive;return true;
}
void Window::set_frame_cap(const double hz)noexcept{storage_->frame_cap_interval_ns=hz>1.?static_cast<Uint64>(1000000000./hz):Uint64{0};}
int Window::drawable_width()const noexcept{return storage_->width;}int Window::drawable_height()const noexcept{return storage_->height;}std::string Window::gpu_driver()const{const char *driver=SDL_GetGPUDeviceDriver(storage_->device);if(!driver)throw sdl_error("SDL GPU driver query failed");return driver;}std::string Window::presentation_mode()const{return storage_->vsync?"vsync":storage_->vsync_unavailable?"display-refresh-fallback":"unbounded";}
