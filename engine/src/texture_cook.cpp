...
Bc1MipLevel compress_level(const Bc1MipLevel&in,TextureFormat f){if(f==TextureFormat::Rgba8)return in;static std::once_flag initialized;std::call_once(initialized,bc7enc_compress_block_init);bc7enc_compress_block_params params;bc7enc_compress_block_params_init(&params);bc7enc_compress_block_params_init_linear_weights(&params);params.m_uber_level=1;
...
CookedTexture cook_texture(const RgbaImage&image,std::string_view category){
 ...
 for(const auto&m:originals)result.mips.push_back(compress_level(m,result.format));
 ...
 if(result.format!=TextureFormat::Rgba8)for(std::size_t l=0;l<originals.size();++l){
 ...
 if(result.rmse>max_rmse||maximum>max_error){result.format=TextureFormat::Rgba8;result.mips=std::move(originals);result.lossless_fallback=true;}
 return result;
}
