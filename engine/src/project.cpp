...
bool create_project(const std::filesystem::path &root, std::string_view name,
  ...
  stub << "// " << display << " - Stellar Engine game host.\n"
       ...
       << "  // Strategic simulation: engine::SimulationExecutor runs your\n"
       << "  // deterministic game systems on LOD tiers - Active every\n"
       << "  // advance(), Dormant only when woken by an event, others on\n"
       << "  // their cadence. advance() once per on_update couples the\n"
       << "  // scheduler to fixed-step time so --fixed-hz runs stay\n"
       << "  // reproducible; ctx.elapsed_ticks tells each task how many\n"
       << "  // ticks passed since it last ran.\n"
       << "  engine::SimulationExecutor sim;\n"
       << "  double score = 0.0;\n"
       << "  sim.add(1, {.run = [&score](\n"
       << "                      const engine::SimulationTickContext &ctx) {\n"
       << "                score += static_cast<double>(ctx.elapsed_ticks) * .5;\n"
       << "              },\n"
       << "              .tier = engine::SimulationTier::Active,\n"
       << "              .domain = \"game\"});\n"
       << "  sim.add(2, {.run = [&score](\n"
       << "                      const engine::SimulationTickContext &ctx) {\n"
       << "                score += static_cast<double>(ctx.elapsed_ticks);\n"
       << "              },\n"
       << "              .tier = engine::SimulationTier::Background,\n"
       << "              .domain = \"game\"});\n\n"
       << "  // Persistence: capture_state()/restore_state() snapshot the\n"
       << "  // scheduler (tick + per-task cadence/wakeup state); tasks are\n"
       << "  // code, so they are re-registered above before restoring. The\n"
       << "  // framework_state_json codec serializes the plain State struct\n"
       << "  // into RuntimeHost's rotating save-data slots - the same contract\n"
       << "  // the campaign frameworks use.\n"
       << "  const auto restore_sim = [&host, &sim] {\n"
       << "    if (const auto blob = host.load_data(\"sim\")) {\n"
       << "      try {\n"
       << "        const auto json = nlohmann::json::parse(*blob);\n"
       << "        engine::SimulationExecutor::State saved;\n"
       << "        engine::from_json(json, saved);\n"
       << "        (void)sim.restore_state(saved);\n"
       << "      } catch (const std::exception &) {\n"
       << "        // Corrupt or incompatible state: start fresh rather than\n"
       << "        // breaking the session on a stale blob.\n"
       << "      }\n"
       << "    }\n"
       << "  };\n"
       << "  restore_sim();\n"
       << "  host.on_event = [&host, &sim, &restore_sim](\n"
       << "      const native_map::InputEvent &event) {\n"
       << "    if (event.type != native_map::InputEventType::KeyPressed) return;\n"
       << "    if (event.key == 0x4000003e) { // F5: alongside the world save\n"
       << "      nlohmann::json json;\n"
       << "      engine::to_json(json, sim.capture_state());\n"
       << "      const auto text = json.dump();\n"
       << "      host.save_data(\"sim\", std::span{\n"
       << "          reinterpret_cast<const std::byte *>(text.data()),\n"
       << "          text.size()});\n"
       << "    } else if (event.key == 0x40000042) { // F9: world load\n"
       << "      restore_sim();\n"
       << "    }\n"
       << "  };\n\n"
       << "  host.on_update = [&host, &fire_down, &mine_down, &sim](\n"
