...
bool create_project(const std::filesystem::path &root, std::string_view name,
  ...
  stub << "// " << display << " - Stellar Engine game host.\n"
       ...
       << "  // Optional audio: audio/bounce.wav|mp3 under the base package's\n"
       << "  // content dir plays when the player entity bounces off a wall.\n"
       << "  engine::audio::AudioOutput audio;\n"
       << "  std::shared_ptr<const engine::audio::AudioClip> bounce_clip;\n"
       << "  for (const auto *name : {\"audio/bounce.wav\", \"audio/bounce.mp3\"}) {\n"
       << "    const auto path = std::filesystem::path(\"packages\") / \""
       << id << "\" / \"content\" / name;\n"
       << "    if (!std::filesystem::is_regular_file(path)) continue;\n"
       << "    try {\n"
       << "      bounce_clip = engine::audio::decode_audio_clip(path);\n"
       << "    } catch (const std::exception &) {\n"
       << "    }\n"
       << "    if (bounce_clip) break;\n"
       << "  }\n\n"
