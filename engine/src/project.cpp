...
bool create_project(const std::filesystem::path &root, std::string_view name,
  ...
  stub << "// " << display << " - Stellar Engine game host.\n"
       ...
       << "  // Cooked content validates if the project has been cooked in the\n"
       << "  // engine tools (COOK writes build/cooked/Content/runtime.stmanifest).\n"
       ...
       << "  const auto cooked_manifest =\n"
       << "      std::filesystem::path(\"build\") / \"cooked\" / \"Content\" /\n"
       << "      \"runtime.stmanifest\";\n"
       << "  if (std::filesystem::is_regular_file(cooked_manifest)) {\n"
       << "    const engine::AssetRegistry cooked_registry(cooked_manifest);\n"
       ...
       << "  world.add(mover, Transform{120.f, 160.f});\n"
       << "  world.add(mover, Velocity{240.f, 150.f});\n\n"
       ...
       << "    if (auto *t = world.get<Transform>(mover)) {\n"
       << "      auto &v = *world.get<Velocity>(mover);\n"
       << "      t->x += v.dx * dt; t->y += v.dy * dt;\n"
       << "      if (t->x < 0 || t->x > w - 96.f) v.dx = -v.dx;\n"
       << "      if (t->y < 0 || t->y > h - 96.f) v.dy = -v.dy;\n"
       << "    }\n\n"
       << "    DrawList draw;\n"
       << "    draw.overlay.push_back(\n"
       << "        FilledRectangle{{0, 0, w, h}, {8, 16, 26, 255}});\n"
       << "    if (const auto *t = world.get<Transform>(mover))\n"
       << "      draw.overlay.push_back(FilledRectangle{\n"
       << "          {t->x, t->y, 96.f, 96.f}, {86, 196, 255, 255}});\n"
