...
bool create_project(const std::filesystem::path &root, std::string_view name,
  ...
  stub << "// " << display << " - Stellar Engine game host.\n"
       ...
       << "struct Named { std::string value; };\n"
       << "struct SpritePath { std::string value; };\n\n"
       << "// Component codecs give World a stable persisted identity per type\n"
       << "// (required for snapshot/restore). PODs serialize as raw bytes,\n"
       << "// string carriers as UTF-8.\n"
       ...
       << "  return v;\n"
       << "}\n"
       << "std::vector<std::uint8_t> encode_named(const Named &n) {\n"
       << "  return {n.value.begin(), n.value.end()};\n"
       << "}\n"
       << "std::vector<std::uint8_t> encode_sprite_path(const SpritePath &s) {\n"
       << "  return {s.value.begin(), s.value.end()};\n"
       << "}\n"
       << "Named decode_named(const std::vector<std::uint8_t> &b) {\n"
       << "  return Named{{b.begin(), b.end()}};\n"
       << "}\n"
       << "SpritePath decode_sprite_path(const std::vector<std::uint8_t> &b) {\n"
       << "  return SpritePath{{b.begin(), b.end()}};\n"
       << "}\n\n"
       << "int main() {\n"
       ...
       << "              std::string(\"" << id << "/content/\") + name)) {\n"
       << "        try {\n"
       << "          return engine::audio::decode_audio_clip(\n"
       << "              cooked_registry->read(*rec), name);\n"
       ...
       << "      encode_pod<Transform>, decode_pod<Transform>);\n"
       << "  world.register_component<Velocity>(\"velocity\",\n"
       << "      encode_pod<Velocity>, decode_pod<Velocity>);\n"
       << "  world.register_component<Extent>(\"extent\",\n"
       << "      encode_pod<Extent>, decode_pod<Extent>);\n"
       << "  world.register_component<Tint>(\"tint\",\n"
       << "      encode_pod<Tint>, decode_pod<Tint>);\n"
       << "  world.register_component<Named>(\"name\", encode_named,\n"
       << "      decode_named);\n"
       << "  world.register_component<SpritePath>(\"sprite\",\n"
       << "      encode_sprite_path, decode_sprite_path);\n"
       ...
       << "  // (scan_content aliases are \"<id>/content/<relative-path>\"); falls\n"
       << "  // back to the loose source file when the project is uncooked.\n"
       ...
       << "    }\n"
       << "  };\n\n"
       << "  // (Re)spawns World entities from a scene document.\n"
       ...
       << "      world.add(entity, Named{s.name});\n"
       << "      if (!s.sprite.empty()) {\n"
       << "        world.add(entity, SpritePath{s.sprite});\n"
       << "        sprites[i] = decode_sprite(s.sprite);\n"
       << "      }\n"
       ...
       << "      if (const auto *n = world.get<Named>(entities[i]);\n"
       << "          n != nullptr && n->value == \"player\")\n"
       << "        player = entities[i];\n"
       << "      if (const auto *sp = world.get<SpritePath>(entities[i]);\n"
