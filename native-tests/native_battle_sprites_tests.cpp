...
int main(int argc,char**argv){
  try{
    ...
    const auto& image=std::get<Image>(draw.overlay.back());
    require(image.rotation_degrees==90.f&&image.clip&&image.clip->height==540,
    ...
    require(std::get<Image>(parked.overlay.front()).resource==image.resource,"Ship instances decoded duplicate resources.");
    std::vector<BattleArtSprite> fleet(100,sprite);DrawList bounded;assets.append(bounded,fleet);
    require(bounded.overlay.size()==32,"Sprite render budget was not enforced.");
    ...
    require(inserted>0&&inserted+1<ordered.overlay.size()&&std::holds_alternative<Image>(ordered.overlay[inserted]),
