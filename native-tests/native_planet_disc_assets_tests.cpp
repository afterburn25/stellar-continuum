...
public:
  explicit FixtureDirectory(const fs::path &source){const auto seed=std::chrono::high_resolution_clock::now().time_since_epoch().count();for(int attempt=0;attempt<100;++attempt){path_=fs::temp_directory_path()/(L"stellar-planet-discs-"+std::to_wstring(seed)+L"-"+std::to_wstring(attempt));if(fs::create_directory(path_))break;}if(path_.empty()||!fs::exists(path_))throw std::runtime_error("Could not create isolated planet-disc fixtures.");fs::create_directory(path_/L"planets");fs::copy_file(source/L"planets"/L"mercury.png",path_/L"planets"/L"mercury.png");fs::copy_file(source/L"planets"/L"neptune.png",path_/L"planets"/L"neptune.png");fs::copy_file(source/L"planets"/L"gaia-world.png",path_/L"planets"/L"gaia-world.png");}
  ~FixtureDirectory(){std::error_code ignored;fs::remove(path_/L"planets"/L"mercury.png",ignored);fs::remove(path_/L"planets"/L"neptune.png",ignored);fs::remove(path_/L"planets"/L"gaia-world.png",ignored);fs::remove(path_/L"planets",ignored);fs::remove(path_,ignored);}
...
int main(int argc,char **argv)try{
  ...
  auto gaia=appearance(1,106,NativeSystemBodyVisualClass::rocky,"gaia-world",0);const auto gaia_disc=assets.image(gaia);require(gaia_disc&&gaia_disc->width()==256,"approved sprite key did not decode the generated disc");require(pixel(*gaia_disc,0,0)[3]==0,"sprite margin was not transparent");gaia.lighting_longitude=2.4f;require(assets.image(gaia)->pixels()==gaia_disc->pixels(),"pre-lit sprite was synthetically darkened by map lighting");auto gaia_hidden=gaia;gaia_hidden.fully_surveyed=false;gaia_hidden.visual_class=NativeSystemBodyVisualClass::unknown_planet;bool sprite_spoof_rejected{};try{(void)assets.image(gaia_hidden);}catch(const std::invalid_argument&){sprite_spoof_rejected=true;}require(sprite_spoof_rejected,"unknown body accepted a spoofed sprite key");
  require(fs::remove(fixtures.path()/L"planets"/L"mercury.png"),"decoded Mercury source remained locked or was missing");require(assets.image(mercury)==mercury_disc,"cached disc attempted to decode its released source again");
  NativePlanetDiscAssets missing_assets(fixtures.path());bool missing_path_reported{};try{(void)missing_assets.image(mercury);}catch(const std::exception &error){missing_path_reported=std::string(error.what()).find("mercury.png")!=std::string::npos;}require(missing_path_reported,"missing planet asset diagnostic omitted the requested path");require(missing_assets.cache_entries()==0,"failed source decode populated the disc cache");
