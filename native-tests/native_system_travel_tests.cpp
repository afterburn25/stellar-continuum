...
int main(int argc,char**argv)try{
  ...
  const float diagonal=std::sqrt(.5F);const std::array lane_list{NativeLocalLaneMarker{.destination_system_id=4,.direction={1,0},.transit_gate={fleet_local_gate_radius,0}},NativeLocalLaneMarker{.destination_system_id=5,.direction={0,1},.transit_gate={0,fleet_local_gate_radius}},NativeLocalLaneMarker{.destination_system_id=6,.direction={1,1},.transit_gate={fleet_local_gate_radius*diagonal,fleet_local_gate_radius*diagonal}},NativeLocalLaneMarker{.destination_system_id=7,.direction={1,.001F},.transit_gate={fleet_local_gate_radius,0}}};const std::array metrics{NativeLaneLabelMetrics{4,184,18},NativeLaneLabelMetrics{5,176,18},NativeLaneLabelMetrics{6,220,18},NativeLaneLabelMetrics{7,196,18}};const auto geometry=layout_local_lanes(spatial,viewport,{-10000,-10000,20000,20000},lane_list,metrics);require(geometry.size()==lane_list.size()&&std::abs(geometry[0].transit_gate.x-(500+100*local_chart_render_radius_factor*fleet_local_gate_radius))<.001F,"decorative geometry moved the canonical transit gate");const auto boundary=local_orbital_boundary_radius(spatial,viewport);require_lane_geometry(geometry,{viewport.center_x,viewport.center_y},boundary);require(hit_local_lane(geometry[0],geometry[0].center),"lane draw and hit triangles disagree");require(local_lane_visible(geometry[0],{0,0,1280,720}),"720p cardinal lane was incorrectly culled");require(!local_lane_visible(geometry[0],{-5000,-5000,100,100}),"offscreen lane was not culled");SystemSpatialViewport viewport4k{1920,1080,2};const auto geometry4k=layout_local_lanes(spatial,viewport4k,{-10000,-10000,20000,20000},lane_list,metrics);require_lane_geometry(geometry4k,{viewport4k.center_x,viewport4k.center_y},local_orbital_boundary_radius(spatial,viewport4k));require(local_lane_visible(geometry4k[0],{0,0,3840,2160}),"4K cardinal lane was incorrectly culled");
  ...
  for(int i=0;i<16;++i){
    ...
    const auto layout=layout_local_lanes(spatial,viewport,{-10000,-10000,20000,20000},lanes,sizes);
  ...
  // Compact fields pull each lane inside the chart so the affordance stays
  // visible and clickable — the workspace swallows input outside world_field.
  for(int i=0;i<16;++i){
    const float angle=i*3.14159265358979323846f/8;
    const std::array lanes{NativeLocalLaneMarker{.destination_system_id=1,.direction={std::cos(angle),std::sin(angle)},.transit_gate={fleet_local_gate_radius*std::cos(angle),fleet_local_gate_radius*std::sin(angle)}}};
    const std::array sizes{NativeLaneLabelMetrics{1,120,16}};
    const UiRect tiny_field{20,10,300,160};
    const auto clamped=layout_local_lanes(spatial,SystemSpatialViewport{170,90,.5f},tiny_field,lanes,sizes);
    const auto&g=clamped.front();
    require(g.bounds.x>=tiny_field.x-.01f&&g.bounds.y>=tiny_field.y-.01f&&g.bounds.x+g.bounds.width<=tiny_field.x+tiny_field.width+.01f&&g.bounds.y+g.bounds.height<=tiny_field.y+tiny_field.height+.01f,"compact field lane escaped the chart bounds");
    require(hit_local_lane(g,g.center),"clamped lane lost its clickable affordance");
  }
  NativeLocalLaneMarker fallback{.destination_system_id=5,.direction={},.transit_gate={fleet_local_gate_radius,0}};const std::array fallback_list{fallback};const std::array fallback_metrics{NativeLaneLabelMetrics{5,40,14}};const auto fallback_geometry=layout_local_lanes(spatial,viewport,{-10000,-10000,20000,20000},fallback_list,fallback_metrics);require(fallback_geometry[0].apex.x>fallback_geometry[0].base_a.x,"zero-length direction did not preserve canonical +X fallback");
