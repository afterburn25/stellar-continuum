...
void execute_contract_row(const fs::path &research_root, const Json &integrated, const Json &row, Json &after, Json &result) {
  ...
  if (name == "move-owner-retains-borrowed-diplomacy") { CampaignFrame first(create_owner_for_row(research_root, named(integrated.at("Rows"), "fresh-zero")), StrategicClock{}, CampaignFramePolicy::Player); CampaignFrame moved(std::move(first)); CampaignFrame assigned(create_owner_for_row(research_root, named(integrated.at("Rows"), "fresh-zero")), StrategicClock{}, CampaignFramePolicy::Player); assigned = std::move(moved); require(assigned.advance(0).strategic_results.size() == 1, "moved owner lost runtime borrowers"); require(!assigned.last_advance_failure(), "successful advance left a stale failure record"); result = {{"StableOwnerRequired", true}, {"CanonicalWorldCount", 1}}; }
  else { CampaignFrame frame(create_owner_for_row(research_root, named(integrated.at("Rows"), row.at("IntegratedRow").get<std::string>())), StrategicClock{}, CampaignFramePolicy::Player); std::size_t completed = 0; bool ready = false, failed = false; try { const auto value = frame.advance(row.at("Input").at("Delta")); completed = value.strategic_results.size(); ready = value.ready_for_save_capture; } catch (const std::exception &) { failed = true; } require(failed, "failed source-integrated step returned a completed frame"); require(frame.last_advance_failure().has_value(), "strategic failure produced no advance-failure record"); const auto &failure = *frame.last_advance_failure(); require(failure.phase == "core" || failure.phase == "sensor" || failure.phase == "research" || failure.phase == "diplomacy" || failure.phase == "chronicle", "advance failure phase unattributed: " + failure.phase); require(!failure.message.empty(), "advance failure lost the exception message"); result = {{"ReadyForSaveCapture", ready}, {"CompletedStrategicResults", completed}}; }
}
