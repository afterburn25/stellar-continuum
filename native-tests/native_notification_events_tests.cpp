...
DiplomaticHistoryEventSnapshot event(std::int64_t id,int target,std::string message,
    DiplomaticEventKind kind=DiplomaticEventKind::proposal_sent){
  DiplomaticHistoryEventSnapshot row;
  row.event_id=id;row.tick=1000;row.kind=kind;
...
void observer_and_activation(){
  ...
  require(feed.items().front().message=="A diplomatic proposal has been sent."&&
      ...
      feed.items().front().severity==NotificationSeverity::Info,
  ...
  view.recent_events.push_back(event(5,2,"WAR SECRET DETAILS",DiplomaticEventKind::war_declared));
  view.recent_events.push_back(event(6,2,"DECLINE SECRET",DiplomaticEventKind::proposal_rejected));
  view.recent_events.push_back(event(7,2,"ACCEPT SECRET",DiplomaticEventKind::proposal_accepted));
  publisher.harvest(feed,view);
  require(feed.items().size()==5&&
      feed.items()[2].severity==NotificationSeverity::Alert&&
      feed.items()[3].severity==NotificationSeverity::Caution&&
      feed.items()[4].severity==NotificationSeverity::Positive,
      "Diplomatic event kinds did not map onto report severity");
...
void chronicle_seeding(){
  ...
  add(412.,"war.battle","Fleet action over Halcyon",{1},0.9,12,{1,7});
  ...
  require(feed.items().size()==4,"Observer-invisible chronicle entry leaked");
  ...
  require(feed.items()[1].category=="Combat"&&
      feed.items()[1].severity==NotificationSeverity::Alert,
      "War chronicle report did not escalate to alert severity");
  require(feed.items()[2].category=="Colony","Category label not mapped");
  ...
  require(feed.items().front().system_id&&*feed.items().front().system_id==9&&
      feed.items()[1].system_id&&*feed.items()[1].system_id==12&&
      !feed.items()[2].system_id,
  ...
  require(feed.items().front().diplomatic_contact_id&&
      ...
      *feed.items()[1].diplomatic_contact_id==7&&
      feed.items()[2].diplomatic_contact_id&&
      *feed.items()[2].diplomatic_contact_id==7&&
...
void bounded_categories(){
  ...
  summary.counts[static_cast<std::size_t>(FeedbackKind::CombatAlert)]=2;
  publish_campaign_notifications(feed,summary,0.);
  require(feed.items().size()==3&&feed.items().front().date=="2050-03-21"&&
      ...
      feed.items().front().severity==NotificationSeverity::Info&&
      feed.items()[1].severity==NotificationSeverity::Positive&&
      feed.items()[2].severity==NotificationSeverity::Alert&&
