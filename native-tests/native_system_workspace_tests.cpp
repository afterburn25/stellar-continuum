...
int main(int argc,char**argv)try{
  ...
  auto launch=std::ranges::find(display.infrastructure,"orbital_launch_complex",&NativeSystemInfrastructureMarker::project_id);
  require(launch!=display.infrastructure.end()&&launch->host_body_id,"fresh Sol view lacks a hosted launch complex marker");
  launch->state=NativeInfrastructureState::active;launch->progress=.5;
  ...
  const auto host=std::ranges::find(projected.bodies,*launch->host_body_id,&SystemSpatialBodyMarker::body_id);
  require(host!=projected.bodies.end(),"hosted launch complex lost its anchor body");
  const auto host_screen=workspace.viewport()->world_to_screen(host->offset_x,host->offset_y);
  const auto bearing=.9f;const Point launch_point{host_screen.x+std::cos(bearing)*(workspace.viewport()->body_radius(*host)+48.f),host_screen.y+std::sin(bearing)*(workspace.viewport()->body_radius(*host)+48.f)};
  require(std::ranges::any_of(draw.world,[&](const WorldCommand&item){const auto*image=std::get_if<Image>(&item);return image&&image->resource&&image->resource->width()==stellar::native_orbital::NativeOrbitalStructureRenderer::texture_size&&std::abs(image->destination.x+image->destination.width*.5f-launch_point.x)<.01f&&std::abs(image->destination.y+image->destination.height*.5f-launch_point.y)<.01f;}),"active launch complex did not draw the staged structure texture");
  require(std::ranges::any_of(draw.world,[](const WorldCommand&item){const auto*label=std::get_if<Text>(&item);return label&&label->value=="LAUNCH COMPLEX";}),"orbital marker did not carry the reference short label");
  auto infra_command=workspace.handle({InputEventType::LeftPressed,launch_point},1280,720);
  require(infra_command.captured&&infra_command.kind==SystemWorkspaceCommandKind::none,"orbital marker click leaked or changed selection");
  (void)workspace.handle({InputEventType::LeftReleased,launch_point},1280,720);
  draw={};workspace.render(draw,1280,720);
  require(std::ranges::any_of(draw.overlay,[](const UiOverlayCommand&item){const auto*label=std::get_if<Text>(&item);return label&&label->value=="ORBITAL CONSTRUCTION";}),"orbital marker click did not focus the inspector");
  require(std::ranges::any_of(draw.overlay,[](const UiOverlayCommand&item){return std::holds_alternative<Image>(item);}),"orbital inspector did not draw the staged preview image");
  const auto manage=center(SystemWorkspaceLayout::for_viewport(1280,720).infrastructure_action);
  infra_command=workspace.handle({InputEventType::LeftPressed,manage},1280,720);
  require(infra_command.captured&&infra_command.kind==SystemWorkspaceCommandKind::open_construction&&infra_command.project_id=="orbital_launch_complex","inspector Manage action did not route to construction with its project");
  ...
  workspace.open(std::move(*local_view.snapshot),1280,720);const auto measurements_before_refresh=measurements;workspace.refresh_travel(std::move(*travel.snapshot),std::nullopt);require(measurements-measurements_before_refresh==static_cast<int>(workspace.travel_snapshot()->lanes.size()),"lane labels were not measured exactly once when the snapshot refreshed");const auto fleet=std::ranges::find(workspace.travel_snapshot()->fleets,99001,&NativeLocalFleetMarker::fleet_id);require(fleet!=workspace.travel_snapshot()->fleets.end(),"owned local fleet was not projected");const auto local_spatial=project_system(*workspace.snapshot());const auto fleet_point=local_fleet_anchor(*fleet,local_spatial,*workspace.viewport());
