    ...
    try {
        ...
        for(const auto& test:fixture.at("Cases")) {
            ...
            operation(test,[&]() -> std::function<void()> {
                ...
                if(kind=="Capacity") { const auto value=colony(test.at("Colony")); const auto actual=surface_building_capacity(value); const Json expected=value.surface_hub_level<=0?Json(0):test.at("Expected"); return [actual,expected,&test]{equal_json(actual,expected,test.at("Name"));}; } // Planetary Command Center foundations now start with zero slots.
                if(kind=="Integration") {
