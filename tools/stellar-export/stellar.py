...
def native_build(preset, env):
    ...
    run([sys.executable, ROOT / "tools/stellar-export/test_native_notification_runtime.py", "-v"], env=test_env)
    run([sys.executable, ROOT / "tools/stellar-export/test_native_developer_runtime.py", "-v"], env=test_env)
    return directory
...
def export(preset_name):
    ...
    try:
        ...
        manifest = {"schemaVersion": 1, "gameVersion": version["gameVersion"], "engineVersion": version["engineVersion"],
                    ...
                    "gameplayParity": False, "shadersRequired": False, "graphicalAssetsRequired": bool(native_client),
