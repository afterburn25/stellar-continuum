param(
    [string]$OutputRoot = 'C:\Users\after\Documents\Codex\2026-09-12\le\outputs\Stellar-Alien-Fleet-0.2.0',
    [string]$LibraryRoot = 'C:\Users\after\Documents\Stellar Engine\Assets'
)
$ErrorActionPreference = 'Stop'
$repoRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..'))
$assetRoot = Join-Path $repoRoot 'assets\models\alien-fleet-v2'
...
foreach ($file in Get-ChildItem -LiteralPath $assetRoot -File -Recurse | Where-Object { $_.Name -notlike '*--metal-inspection.png' }) {
...
foreach ($file in Get-ChildItem -LiteralPath $PSScriptRoot -File | Where-Object { $_.Extension -in @('.mjs','.json','.ps1','.html') -or $_.Name -in @('.gitignore','.gdignore') }) { Copy-Item -LiteralPath $file.FullName -Destination (Join-Path $sourceRoot $file.Name) }
$sourceNote = @'
...
First copy the delivered Textures and References directories into
assets/models/alien-fleet-v2, and ART-PROVENANCE.md to
docs/art/ALIEN_FLEET_METAL_PROVENANCE.md. The embedded runtime texture data is
included; prepare-metal-texture.mjs can regenerate it from the retained source.
The builder writes assets/models/alien-fleet-v2 relative to that workspace.
...
foreach ($entry in $library) {
    if ($entry.Description -like 'Stellar Continuum / Alien Fleet 0.1.0 /*') {
        $entry.Description = 'Stellar Continuum / Alien Fleet 0.1.0 / Superseded blockout; use the 0.2.0 metal construction candidates for current visual review'
        $superseded++
    }
}
$added = @()
$records = @()
$importFiles = @(Get-ChildItem -LiteralPath $assetRoot -File -Recurse | Where-Object { $_.Extension -in @('.glb','.png','.json','.txt','.md') -and $_.Name -notlike '*--metal-inspection.png' }) + @(Get-Item -LiteralPath (Join-Path $OutputRoot 'README.md'))
foreach ($file in $importFiles) {
    ...
    if (-not @($library | Where-Object { $_.Id -eq $hash -and $_.Name -eq $file.Name }).Count) {
        $description = if ($file.Directory.Name -eq 'References') { 'Stellar Continuum / Alien Fleet 0.2.0 / Generated metal construction design reference; not a rendered 3D model or game screenshot' } else { 'Stellar Continuum / Alien Fleet 0.2.0 / Metal construction candidate; final art review and game integration pending' }
        $library += [pscustomobject][ordered]@{Id=$hash;Name=$file.Name;Category=$category;Size=$file.Length;RelativePath=$relative;Description=$description}
        $added += $file.Name
    }
...
if ($added.Count -gt 0 -or $superseded -gt 0) {
...
$receipt = [ordered]@{version='0.2.0';libraryRoot=$LibraryRoot;before=$beforeCount;after=$library.Count;added=$added.Count;supersededBlockoutEntries=$superseded;assets=$records}
...
[IO.File]::WriteAllText((Join-Path $OutputRoot 'package-manifest.json'), ([ordered]@{version='0.2.0';files=$packRecords} | ConvertTo-Json -Depth 10), $utf8)
$zip = $OutputRoot + '.zip'
